...
import { ChevronLeft, Receipt, ShieldAlert } from 'lucide-react-native';
...
export default function OwnerBillsScreen() {
  ...
  if (!canViewInvoices) {
    return (
      <View className="flex-1 bg-notion-bg-secondary items-center justify-center p-6">
        ...
        <Text className="text-notion-text-secondary text-center mt-2">You don't have permission to view invoices.</Text>
  ...
  return <OwnerBillsContent />;
}

function OwnerBillsContent() {
  const router = useRouter();
  const { light } = useHaptics();
  const { data: invoices, isLoading, isError, refetch, isRefetching } = useQuery({
    queryKey: ['owner_bills'],
    queryFn: async () => {
      const res = await api.invoices.get({ query: { limit: '50' } });
  ...
  return (
    <View className="flex-1 bg-notion-bg-secondary dark:bg-notion-bg">
      <View className="bg-notion-bg dark:bg-notion-bg-secondary border-b border-notion-border pt-14 pb-4 px-4 flex-row items-center">
        ...
        <Heading className="text-notion-text">Invoices & Bills</Heading>
      </View>
        ...
        >
          {invoices?.length === 0 ? (
            ...
            invoices?.map((inv: any) => (
              <Card variant="elevated" key={inv.id} padding="md" className="mb-4 border-notion-border">
                <View className="flex-row justify-between mb-2">
                  <Text className="font-bold text-notion-text text-lg">{inv.invoiceNumber}</Text>
                  <View className="px-2 py-1 bg-notion-green/10 rounded">
                    <Caption className="text-notion-green font-bold uppercase">{inv.status}</Caption>
                  </View>
                </View>
                {inv.booking && (
                  <Text className="text-notion-text-secondary mb-1">Guest: {inv.booking.guestName}</Text>
                )}
                <Text className="text-notion-text-secondary font-mono text-xs mb-3">
                  {new Date(inv.createdAt).toLocaleString()}
                </Text>
                <View className="pt-3 border-t border-notion-border flex-row justify-between">
                  <Text className="text-notion-text font-medium">Grand Total</Text>
                  <Text className="text-notion-text font-bold text-lg">{formatAmount(inv.grandTotal)}</Text>
                </View>
              </Card>
            ))
          )}
