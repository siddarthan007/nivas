...
interface Section {
    ...
    permission?: string;
    children?: { id: string; label: string; permission?: string }[];
}
...
const SECTIONS: Section[] = [
    { id: 'overview', label: 'Dashboard', icon: BarChart3 },
    {
        ...
        icon: FileText,
        children: [
            { id: 'invoices', label: 'Invoices', permission: 'finance:view_records' },
            { id: 'outstanding', label: 'Outstanding / Credit', permission: 'finance:view_records' },
            { id: 'payments', label: 'Payments', permission: 'finance:view_records' },
            { id: 'credit-notes', label: 'Credit Notes', permission: 'finance:view_records' },
            { id: 'customer-ledger', label: 'Customer Ledger', permission: 'finance:view_records' },
        ],
    },
    {
        id: 'accounting',
        label: 'Accounting',
        icon: BookOpen,
        permission: 'finance:view_records',
        children: [
            { id: 'general-ledger', label: 'General Ledger', permission: 'finance:view_records' },
            { id: 'transactions', label: 'Transaction History', permission: 'finance:view_records' },
            { id: 'profit-loss', label: 'Profit & Loss', permission: 'analytics:view_financials' },
            { id: 'balance-sheet', label: 'Balance Sheet', permission: 'analytics:view_financials' },
        ],
    },
    {
        id: 'operations',
        ...
        icon: Clock,
        children: [
            { id: 'shifts', label: 'Shifts', permission: 'finance:view_records' },
            { id: 'night-audit', label: 'Night Audit', permission: 'operations:run_night_audit' },
        ],
    },
    { id: 'exports', label: 'Exports & Compliance', icon: Download, permission: 'finance:view_records' },
];
...
export default function FinanceSidebar({ activeTab, onTabChange }: FinanceSidebarProps) {
    ...
    const visibleSections = SECTIONS.filter(section => {
        ...
        if (section.children) {
            const visibleChildren = section.children.filter(c => !c.permission || can(c.permission));
