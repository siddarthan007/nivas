...
async function request<T>(
    ...
    // Normalize endpoint (ensure leading slash) — must be before guest/staff token selection
    const normalizedEndpoint = endpoint.startsWith('/') ? endpoint : `/${endpoint}`;
    const url = `${API_BASE_URL}${normalizedEndpoint}`;

    // Add auth token if available and not skipped.
    // Guest portal routes use a separate token so staff sessions are not overwritten.
    if (!skipAuth) {
        const isGuestApi = normalizedEndpoint.startsWith('/guest')
            || normalizedEndpoint.startsWith('/billing/guest');
    ...
    while (attempt <= retries) {
        try {
            ...
            if (!response.ok) {
                ...
                // Handle 401 - try a one-time token refresh before giving up (staff sessions only).
                ...
                if (response.status === 403 && data.code === 'LICENSE_INVALID') {
                    const body = data as unknown as LicenseErrorInfo & { data?: LicenseErrorInfo };
                    const nested = body.data;
                    licenseEvents.emit({
                        licenseStatus: body.licenseStatus ?? nested?.licenseStatus ?? 'EXPIRED',
                        message: data.message || 'License is invalid',
                        expiresAt: body.expiresAt ?? nested?.expiresAt,
                        graceEndsAt: body.graceEndsAt ?? nested?.graceEndsAt,
                    });
                }
