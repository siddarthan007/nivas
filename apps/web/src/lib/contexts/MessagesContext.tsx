...
export function MessagesProvider({ children }: { children: ReactNode }) {
    ...
    useEffect(() => {
        const handleMessage = (payload: any) => {
            if (!hasHotel()) return;
            fetchConversations();
            const activeId = activeConversation?.participantId;
            const incomingFrom = payload?.senderId || payload?.participantId;
            if (activeId && (incomingFrom === activeId || payload?.receiverId)) {
                refreshActiveMessages(activeId);
            }
        };
        const unsubs = [
            ws.on('NEW_MESSAGE', handleMessage),
            ws.on('NOTIFICATION', (data: any) => {
                if (data?.notifType === 'NEW_MESSAGE' || data?.type === 'NEW_MESSAGE') {
                    handleMessage(data);
                }
            }),
        ];
