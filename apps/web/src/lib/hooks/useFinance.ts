...
export function useFinance() {
    ...
    const [paymentLimit, setPaymentLimit] = useState(50);
    const [creditNoteLimit, setCreditNoteLimit] = useState(50);
    const [currentShift, setCurrentShift] = useState<Shift | null>(null);
    const [mutationLoading, setMutationLoading] = useState(false);
    const [error, setError] = useState<string | null>(null);

    const invoicesQuery = useFinanceInvoicesQuery(invoiceLimit);
    const paymentsQuery = useFinancePaymentsQuery(paymentLimit);
    const creditNotesQuery = useFinanceCreditNotesQuery(creditNoteLimit);
    ...
    const fetchInvoices = useCallback(async (limit = 50) => {
        ...
        await queryClient.invalidateQueries({ queryKey: queryKeys.finance.invoices(limit) });
    }, [queryClient]);
    ...
    const fetchPayments = useCallback(async (limit = 50) => {
        ...
        await queryClient.invalidateQueries({ queryKey: queryKeys.finance.payments(limit) });
    }, [queryClient]);
    ...
    const fetchCreditNotes = useCallback(async (limit = 50) => {
        setCreditNoteLimit(limit);
        await queryClient.invalidateQueries({ queryKey: queryKeys.finance.creditNotes(limit) });
    }, [queryClient]);
    ...
    const endShift = async (endCashCount: number, notes?: string) => {
        setMutationLoading(true);
        try {
            const res = await api.post<ShiftEndResponse>('/finance/shifts/end', {
                endCashCount,
                notes
            });
...
export function useFinanceDashboard() {
    ...
    const fetchDashboardData = useCallback(async () => {
        ...
        try {
            ...
            const revData: RevenueData = revRes.status === 'fulfilled' && revRes.value.data ? (revRes.value.data as unknown as RevenueData) : {};
            ...
            if (Array.isArray(tbData)) {
                tbData.forEach((account: TrialBalanceAccount) => {
            ...
            setSummary({
                ...
                revenueTrend: (revData?.trend || []).map(p => ({
                    date: p.date,
                    revenue: p.amount,
                })),
            });
