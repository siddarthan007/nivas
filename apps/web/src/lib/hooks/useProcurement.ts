...
function normalizePurchaseOrder(raw: BackendPurchaseOrder): PurchaseOrder {
    const items = (raw.items || []).map((item, idx) => ({
        ...
        receivedQuantity: item.quantityReceived != null ? Number(item.quantityReceived) : undefined,
    }));
...
function buildStats(orders: PurchaseOrder[]): ProcurementStats {
    return {
        ...
        approved: orders.filter(order => order.status === 'APPROVED' || order.status === 'PARTIALLY_RECEIVED').length,
...
export function useProcurement(): UseProcurementReturn {
    ...
    const receivePO = useCallback(async (poId: number): Promise<boolean> => {
        try {
            ...
            toast.success('Goods received (GRN posted)');
            await fetchPurchaseOrders();
            return true;
        } catch (e) {
