...
export function useReports() {
    ...
    const fetchReports = useCallback(async () => {
        ...
        try {
            const rangeQs = `startDate=${period.startDate}&endDate=${period.endDate}`;
            const [revenueRes, occupancyRes, metricsRes] = await Promise.all([
                api.get<RevenueAnalytics>(`/analytics/revenue?${rangeQs}`),
                api.get<OccupancyAnalytics>(`/analytics/occupancy?${rangeQs}`),
                api.get<KeyMetrics>(`/analytics/key-metrics?${rangeQs}`),
            ]);
