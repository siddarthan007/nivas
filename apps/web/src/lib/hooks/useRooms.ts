...
export function useRooms() {
    ...
    useEffect(() => {
        const unsubs = [
            ws.on('CHECKOUT_ROOM_READY', fetchRooms),
            ws.on('HOUSEKEEPING_TASK_COMPLETED', fetchRooms),
            ws.on('BOOKING_CHECKED_IN', fetchRooms),
            ws.on('BOOKING_CHECKED_OUT', fetchRooms),
        ];
