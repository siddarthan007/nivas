...
import { isWidgetAllowed, isQuickActionAllowed } from '@/lib/config/property-modules';
...
const ACTION_MODULE_MAP: Record<string, string> = {
    ...
    '/hotel/inventory?tab=procurement': 'inventory',
};
...
const HOTEL_WIDGETS: Record<string, WidgetConfig[]> = {
    ...
    ACCOUNTANT: [
        ...
        },
        {
            ...
            color: 'purple',
            getValue: () => 'Manage',
            href: '/hotel/inventory?tab=procurement'
        },
    ],
    DEFAULT: [
        {
...
const ROLE_QUICK_ACTIONS: Record<string, QuickAction[]> = {
    ...
    ACCOUNTANT: [
        ...
        { label: 'Procurement', icon: Package, href: '/hotel/inventory?tab=procurement', color: 'orange' },
    ],
};
...
export default function DashboardPage() {
    ...
    const widgets = useMemo((): WidgetConfig[] => {
        ...
        return base.filter(w => {
            if (!isWidgetAllowed(w.id, moduleConfig)) return false;
            const mod = WIDGET_MODULE_MAP[w.id];
            if (mod === undefined) return true;
            return mod === null || hasModule(mod);
        });
    }, [user, isUserType, roleKey, hasModule, moduleConfig]);
    ...
    const quickActions = useMemo((): QuickAction[] => {
        ...
        return base.filter(a => {
            if (!isQuickActionAllowed(a.href, moduleConfig)) return false;
    ...
    return (
                    ...
                    {!isUserType('SUPER_ADMIN') && user?.hotelId && (
                        <>
                            ...
                            {can('analytics:view_operations') && moduleConfig.enableHotel && <ForecastWidget />}
                        </>
                    )}
                    ...
                    {/* Sales Trend + Insights (finance/analytics roles only) */}
                    {!isUserType('SUPER_ADMIN') && can('analytics:view_financials') && <SalesTrendCard />}
                    {!isUserType('SUPER_ADMIN') && can('analytics:view_operations') && <SalesInsights />}
