...
function CreateBookingModal({
    ...
    prefill,
    initialGuestId,
    checkInNowDefault = false,
}: {
    isOpen: boolean;
    onClose: () => void;
    onCreate: (data: CreateBookingPayload, options?: { checkInNow?: boolean }) => Promise<void>;
    bookings: Booking[];
    prefill?: { roomId?: number; checkIn?: string };
    initialGuestId?: string;
    checkInNowDefault?: boolean;
}) {
    ...
    const applyGuestToForm = (guest: GuestSearchResult) => {
        setSelectedGuestId(guest.id);
        setIsExistingGuest(true);
        setFormData(prev => ({
            ...prev,
            guestId: guest.id,
            guestName: guest.fullName || prev.guestName,
    ...
    useEffect(() => {
        if (isOpen) {
            setStep(1);
            setCheckInNow(checkInNowDefault);
            ...
            if (initialGuestId) {
                GuestService.getById(initialGuestId).then(guest => {
                    if (guest) {
    ...
    }, [isOpen, prefill, initialGuestId, checkInNowDefault]);
    ...
    const handleGuestSelect = async (guest: GuestSearchResult) => {
        const full = await GuestService.getById(guest.id);
        applyGuestToForm(full || guest);
    };
    ...
    const handleSubmit = async () => {
        ...
        try {
            ...
            await onCreate({ ...formData, guestName } as CreateBookingPayload, { checkInNow });
            onClose();
        } finally {
            setLoading(false);
        }
    ...
    const canProceedStep2 = (formData.guestName || formData.firstName) && formData.guestPhone;
    ...
    return (
        <Modal isOpen={isOpen} onClose={onClose} title="New Booking" size="lg" onSubmit={step === 3 ? handleSubmit : undefined}>
            {/* Step Indicator */}
                ...
                <div style={{ display: 'flex', flexDirection: 'column', gap: 'var(--space-4)', animation: 'fadeIn 0.2s ease-out' }}>
                    ...
                    <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 'var(--space-3)' }}>
                        <div>
                            ...
                            <Input placeholder="First name" value={formData.firstName || ''}
                                onChange={e => setFormData({ ...formData, firstName: e.target.value, guestName: `${e.target.value} ${formData.lastName || ''}`.trim() })}
                            />
                        </div>
                        <div>
                            <label style={labelStyle}>Last Name</label>
                            <Input placeholder="Last name" value={formData.lastName || ''}
                                onChange={e => setFormData({ ...formData, lastName: e.target.value, guestName: `${formData.firstName || ''} ${e.target.value}`.trim() })}
                            />
                        </div>
                    </div>
                    ...
                    <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 'var(--space-3)' }}>
                        <div>
                            ...
                            <Input placeholder="Father's name" value={formData.fatherName || ''}
                    ...
                    <div>
                        ...
                        <Input placeholder="Street, area, postal code" value={formData.address || ''}
                    ...
                    <div>
                        ...
                        <Input placeholder="Guest notes..." value={formData.notes || ''}
            ...
            {step === 3 && (
                <div style={{ display: 'flex', flexDirection: 'column', gap: 'var(--space-4)', animation: 'fadeIn 0.2s ease-out' }}>
                    ...
                    <div style={{
                        padding: 'var(--space-3)',
                        backgroundColor: checkInNow ? 'var(--notion-green-bg)' : 'var(--notion-bg-secondary)',
                        borderRadius: 'var(--radius-md)',
                        border: `1px solid ${checkInNow ? 'var(--notion-green)' : 'var(--notion-border)'}`,
                        display: 'flex',
                        alignItems: 'center',
                        ...
                        <div>
                            <div style={{ fontSize: '13px', fontWeight: 600, color: 'var(--notion-text)' }}>Check in immediately</div>
                            <div style={{ fontSize: '12px', color: 'var(--notion-text-secondary)', marginTop: '2px' }}>
                                Mark guest as in-house right after creating this booking
                            </div>
                        </div>
                        <button
                            type="button"
                            ...
                            style={{
                                ...
                                backgroundColor: checkInNow ? 'var(--notion-green)' : 'var(--notion-border)',
                                position: 'relative', transition: 'background-color 0.2s',
                            }}
                        >
                            <span style={{
                                position: 'absolute', top: '3px',
                                left: checkInNow ? '23px' : '3px',
            ...
            <div style={{ display: 'flex', gap: 'var(--space-2)', justifyContent: 'space-between', marginTop: 'var(--space-5)' }}>
                ...
                >
                    {step === 1 ? (
                        <>
                            <span style={{ display: 'inline-flex', alignItems: 'center', lineHeight: 0 }}><XCircle size={16} /></span>
                            Cancel
                        </>
                    ) : (
                        <>
        ...
        <Modal isOpen={isOpen} onClose={onClose} title="Edit Booking" onSubmit={() => { (document.getElementById('edit-booking-form') as HTMLFormElement)?.requestSubmit(); }}>
            <form id="edit-booking-form" onSubmit={handleSubmit} style={{ display: 'flex', flexDirection: 'column', gap: 'var(--space-4)' }}>
                ...
                <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 'var(--space-3)' }}>
                    ...
                    <div>
                        <label className="block text-xs text-[var(--notion-text-secondary)] mb-1">Advance Paid (NPR)</label>
                        <Input type="number" value={formData.advancePayment ?? ''} onChange={e => setFormData({ ...formData, advancePayment: e.target.value === '' ? 0 : Number(e.target.value) })} />
                    </div>
                </div>
                <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 'var(--space-3)' }}>
                    <div>
                        <label className="block text-xs text-[var(--notion-text-secondary)] mb-1">Guests</label>
                        <Input type="number" min={1} value={formData.guestCount ?? ''} onChange={e => setFormData({ ...formData, guestCount: e.target.value === '' ? 1 : Number(e.target.value) })} />
                    </div>
                    <div>
                ...
                <div>
                    <label className="block text-xs text-[var(--notion-text-secondary)] mb-1">Notes</label>
                    <Input value={formData.notes || ''} onChange={e => setFormData({ ...formData, notes: e.target.value })} placeholder="Special requests, internal notes..." />
                </div>
...
function ChangeRoomModal({
    ...
    useEffect(() => {
        if (isOpen) {
            setSelectedRoom(booking?.roomId ?? null);
        }
    }, [isOpen, booking?.id, booking?.roomId]);
...
export default function BookingsPage() {
    ...
    const [initialGuestId, setInitialGuestId] = useState<string | undefined>(undefined);
    const [checkInNowDefault, setCheckInNowDefault] = useState(false);
    ...
    useEffect(() => {
        if (searchParams.get('action') === 'new') {
            ...
            const checkInNow = searchParams.get('checkInNow') === '1';
            if (roomId || checkIn) setCreatePrefill({ roomId, checkIn });
            if (guestId) setInitialGuestId(guestId);
            setCheckInNowDefault(checkInNow);
    ...
    return (
                                        ...
                                        <tr key={booking.id} style={{ borderBottom: '1px solid var(--notion-border)' }}>
                                            ...
                                            <td style={{ padding: '12px 16px' }}>
                                                <div style={{ display: 'flex', gap: '6px', flexWrap: 'wrap', alignItems: 'center' }}>
                                                    ...
                                                    {booking.status === 'CHECKED_IN' && can('guests:check_out') && (
                ...
                <CreateBookingModal
                    isOpen={isCreateOpen}
                    onClose={() => { setIsCreateOpen(false); setCreatePrefill(undefined); setInitialGuestId(undefined); setCheckInNowDefault(false); }}
                    onCreate={async (data, options) => {
                        const booking = await createBooking(data);
                        if (options?.checkInNow && booking?.id) {
                    ...
                    checkInNowDefault={checkInNowDefault}
                />
