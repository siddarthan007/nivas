...
export default function CorporatePage({
    ...
    const [activeTab, setActiveTab] = useState<'companies' | 'agents'>(initialTab === 'agents' ? 'agents' : 'companies');

    useEffect(() => {
        setActiveTab(initialTab === 'agents' ? 'agents' : 'companies');
    }, [initialTab]);
    ...
    const inner = (
        <>
            <PageContainer>
                <div style={{ padding: embedded ? 0 : 'var(--space-6)', maxWidth: '1400px', margin: '0 auto' }}>
                    {/* Header — hidden when embedded (Customers hub provides navigation) */}
                    {!embedded && (
                    ...
                    {/* Tabs — hidden when embedded in Customers hub (hub tabs handle navigation) */}
                    {!embedded && <TabNav activeTab={activeTab} onTabChange={(tab) => setActiveTab(tab as 'companies' | 'agents')} />}
