...
function BookingCard({ booking, onUpdateStatus, onDelete, onGenerateInvoice, isGeneratingInvoice }: {
    ...
    return (
        <div style={{ backgroundColor: 'var(--notion-bg-secondary)', borderRadius: 'var(--radius-lg)', border: '1px solid var(--notion-border)', padding: 'var(--space-4)' }}>
            ...
            <div style={{ fontSize: '13px', marginBottom: 'var(--space-3)' }}>
                ...
                {booking.guest && (
                    <div style={{ display: 'flex', alignItems: 'center', gap: '4px', marginTop: '4px', color: 'var(--notion-blue)' }}>
                        <User size={12} />
                        <a href={`/hotel/guests/${booking.guest.id}`} style={{ color: 'var(--notion-blue)', textDecoration: 'none' }}>{booking.guest.fullName}</a>
                    </div>
                )}
                {booking.invoice && (
            ...
            <div style={{ display: 'flex', gap: 'var(--space-2)', flexWrap: 'wrap' }}>
                {can('banquets:update') && booking.status === 'PENDING' && (
                    <>
                ...
                {can('banquets:update') && booking.status === 'CONFIRMED' && (
                    <>
                        <Button size="sm" onClick={() => onUpdateStatus('COMPLETED')}>Mark Completed</Button>
                        <Button size="sm" variant="secondary" onClick={() => onUpdateStatus('CANCELLED')}>Cancel</Button>
                    </>
                )}
                {can('finance:generate_invoice') && !booking.invoice && (booking.status === 'CONFIRMED' || booking.status === 'COMPLETED') && onGenerateInvoice && (
                    <Button size="sm" variant="secondary" onClick={onGenerateInvoice} disabled={isGeneratingInvoice}>
...
function CreateVenueModal({ isOpen, onClose, onSubmit }: {
    ...
    useEffect(() => {
        if (isOpen) {
            setFormData({ name: '', capacity: 50, description: '', amenities: [] });
        }
    }, [isOpen]);
...
export default function EventsPage() {
    ...
    const [statusFilter, setStatusFilter] = useState<BanquetBookingStatus | 'all'>('all');
    ...
    const filteredBookings = useMemo(() => {
        ...
        return data;
    }, [bookings, searchQuery, statusFilter, dateFrom, dateTo, bookingSort, bookingSortDir]);
    ...
    return (
                ...
                <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: 'var(--space-6)' }}>
                    ...
                    <div style={{ display: 'flex', gap: 'var(--space-3)' }}>
                        ...
                        {activeTab === 'venues' && can('banquets:create') && <Button onClick={() => setIsVenueModalOpen(true)}><Plus size={14} style={{ marginRight: '6px' }} />Add Venue</Button>}
                        {activeTab === 'bookings' && venues.length > 0 && can('banquets:create') && <Button onClick={() => setIsBookingModalOpen(true)}><Plus size={14} style={{ marginRight: '6px' }} />New Booking</Button>}
                    </div>
                </div>
                    ...
                    <>
                        {bookings.length > 0 && (
                            <div style={{ display: 'flex', gap: 'var(--space-3)', marginBottom: 'var(--space-4)', flexWrap: 'wrap', alignItems: 'center' }}>
                                ...
                                <select value={statusFilter} onChange={e => setStatusFilter(e.target.value as BanquetBookingStatus | 'all')}
                            ...
                            {bookings.length === 0 ? (
                                ...
                                filteredBookings.map(booking => (
                                    <BookingCard
                                        ...
                                        onGenerateInvoice={() => handleGenerateInvoice(booking.id)}
                                        isGeneratingInvoice={generatingInvoiceId === booking.id}
                                    />
                                ))
                            )}
                        </div>
