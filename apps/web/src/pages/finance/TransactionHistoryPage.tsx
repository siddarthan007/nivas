...
interface GlTxn { id: string; date: string; description: string; reference?: string; direction: 'in' | 'out'; amount: number; category?: string }
...
export default function TransactionHistoryPage(_props: any) {
    ...
    const load = async () => {
        ...
        try {
            ...
            if (dateFrom) searchParams.append('from', dateFrom);
            if (dateTo) searchParams.append('to', dateTo);
            const qs = searchParams.toString();
            const res = await api.get<GlTxn[]>(`/finance/gl/cash-transactions${qs ? `?${qs}` : ''}`);
            setRows(res.data || []);
                    ...
                    <table style={{ width: '100%', borderCollapse: 'collapse', fontSize: '13px' }}>
                        <thead>
                            <tr style={{ borderBottom: '1px solid var(--notion-divider)', backgroundColor: 'var(--notion-bg-secondary)' }}>
                                ...
                                <th style={{ padding: '10px 14px', textAlign: 'left', color: 'var(--notion-text-secondary)', fontWeight: 500, fontSize: '11px', textTransform: 'uppercase' }}>Category</th>
                        ...
                        <tbody>
                            {filtered.length === 0 ? (
                                <tr><td colSpan={7}><EmptyState title="No transactions found" description="Try adjusting your filters or date range." /></td></tr>
                            ) : (
                                filtered.map(tx => (
                                    <tr key={tx.id} style={{ borderBottom: '1px solid var(--notion-divider)' }}>
                                        ...
                                        <td style={{ padding: '8px 14px' }}>
                                            <span style={{
                                                fontSize: '11px', fontWeight: 600, padding: '2px 8px', borderRadius: 'var(--radius-sm)',
                                                backgroundColor: 'var(--notion-blue-bg)', color: 'var(--notion-blue)', textTransform: 'capitalize',
                                            }}>
                                                {tx.category === 'lodging' ? 'Lodging' : tx.category}
                                            </span>
                                        </td>
