...
function PortalDashboard({
    ...
    fetchRequests: () => void;
    onLogout: () => void;
    onToggleDnd: (enabled: boolean) => Promise<boolean>;
}) {
    ...
    const dndEnabled = !!portalConfig?.dndEnabled;
    ...
    const tabs = [
        ...
        ...(portalConfig?.showBillBreakdown !== false ? [{ id: 'bills' as const, label: 'Bills', icon: Receipt }] : []),
    ...
    return (
        <div style={{
            ...
            <div style={{ maxWidth: '600px', margin: '0 auto', padding: 'var(--space-4)' }}>
                {activeTab === 'home' && (
                    <>
                        ...
                        {/* Do Not Disturb */}
                        <div style={{
                            backgroundColor: portalConfig?.dndEnabled ? 'var(--notion-red-bg)' : 'var(--notion-bg-secondary)',
                            borderRadius: 'var(--radius-lg)',
                            border: `1px solid ${portalConfig?.dndEnabled ? 'var(--notion-red)' : 'var(--notion-border)'}`,
                            padding: 'var(--space-4)',
                            marginBottom: 'var(--space-5)',
                            display: 'flex',
                            alignItems: 'center',
                            ...
                            <div style={{ display: 'flex', alignItems: 'center', gap: 10 }}>
                                <DoorClosed size={18} style={{ color: portalConfig?.dndEnabled ? 'var(--notion-red)' : 'var(--notion-text-secondary)' }} />
                                <div>
                                    <div style={{ fontSize: 14, fontWeight: 600, color: 'var(--notion-text)' }}>Do Not Disturb</div>
                                    <div style={{ fontSize: 12, color: 'var(--notion-text-secondary)' }}>
                                        {portalConfig?.dndEnabled ? 'Staff will not enter your room' : 'Housekeeping may visit when you are away'}
                                    </div>
                                </div>
                            </div>
                            <button
                                ...
                                style={{
                                    ...
                                    borderRadius: 14,
                                    border: 'none',
                                    cursor: dndBusy ? 'wait' : 'pointer',
                                    background: dndEnabled ? 'var(--notion-red)' : 'var(--notion-bg-tertiary)',
                                    position: 'relative',
                        ...
                        {portalConfig && (
                            <div style={{
                                ...
                                {Object.keys(portalConfig.socialLinks || {}).length > 0 && (
                                    <div style={{ marginBottom: 'var(--space-3)' }}>
                                        <div style={{ fontSize: '12px', fontWeight: '600', color: 'var(--notion-text-secondary)', marginBottom: 'var(--space-2)', display: 'flex', alignItems: 'center', gap: '4px' }}>
                                            <Globe size={12} />
                                            Follow Us
                                        ...
                                        <div style={{ display: 'flex', flexWrap: 'wrap', gap: 'var(--space-2)' }}>
                                            {Object.entries(portalConfig.socialLinks).filter(([, url]) => url?.trim()).map(([platform, url]) => (
                                                <a
                                                    key={platform}
                        ...
                        {(portalConfig?.showOrderProgress !== false) && pendingRequests.length > 0 && (
                            <div style={{ marginBottom: 'var(--space-5)' }}>
                                <h2 style={{
                                    fontSize: '16px',
                                    fontWeight: '600',
                    ...
                    <>
                        ...
                        {Object.keys(menuByCategory).length === 0 ? (
                            <div style={{
                                ...
                                <UtensilsCrossed size={40} style={{ opacity: 0.3, marginBottom: 'var(--space-3)' }} />
                                <p>Menu is not available right now</p>
                                <p style={{ fontSize: '13px', marginTop: 'var(--space-2)' }}>Please contact the front desk to order.</p>
                            </div>
            ...
            `}</style>
            <AiChatLauncher
                endpoint="/guest/actions/concierge"
                field="message"
                sendHistory
                title="Hotel Concierge"
                subtitle="Order food, request service, ask about amenities"
                intro="Hi! I can help you order room service, request housekeeping, or answer questions about the hotel."
                suggestions={['What is on the menu?', 'Extra towels please', 'What time is checkout?']}
            />
        </div>
    );
}
