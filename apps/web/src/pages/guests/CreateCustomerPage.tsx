...
export default function CreateCustomerPage() {
  ...
  const handleSubmit = async (redirectToCheckIn = false) => {
    ...
    try {
      ...
      toast.success(redirectToCheckIn ? 'Customer created — continue with check-in' : 'Customer created successfully');
      if (redirectToCheckIn && guest?.id) {
        ...
        router.push(`/hotel/bookings/checkin?${params.toString()}`);
        return;
      }
            ...
            <div style={{ display: 'flex', flexDirection: 'column', gap: 'var(--space-4)' }}>
              ...
              <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 'var(--space-3)' }}>
                <div>
                  ...
                  <DateField value={form.dob} onChange={v => update('dob', v)} />
                </div>
                <div>
        ...
        <div style={{ display: 'flex', justifyContent: 'space-between', marginTop: 'var(--space-6)' }}>
          ...
          {step < 3 ? (
            ...
            <div style={{ display: 'flex', gap: 8 }}>
              <Button variant="secondary" onClick={() => handleSubmit(false)} disabled={isSubmitting || !canProceed()}>
                <Check size={14} style={{ marginRight: '4px' }} />
                {isSubmitting ? 'Creating...' : 'Save Customer'}
              </Button>
              {(form.customerType === 'HOTEL_GUEST' || form.customerType === 'BOTH') && (
                <Button onClick={() => handleSubmit(true)} disabled={isSubmitting || !canProceed() || !form.phone.trim()}>
                  <Check size={14} style={{ marginRight: '4px' }} />
                  {isSubmitting ? 'Creating...' : 'Save & Check In'}
                </Button>
              )}
            </div>
          )}
