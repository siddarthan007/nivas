...
import { ArrowLeft, Edit3, Save, Star, Ban, Phone, Mail, Globe, MapPin, Shield, Calendar, CreditCard, FileText, UtensilsCrossed, Home, DollarSign, Loader2, User, DoorOpen } from 'lucide-react';
...
export default function CustomerDetailPage() {
    ...
    useEffect(() => {
        if (guestId === 'new') {
            router.replace('/hotel/guests/new');
        }
    }, [guestId, router]);
    ...
    if (loading) return (
        <div style={{ padding: 'var(--space-12)', display: 'flex', justifyContent: 'center', alignItems: 'center', gap: '12px' }}>
            ...
            <div style={{ padding: 'var(--space-8)' }}>
                ...
                <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: 'var(--space-6)' }}>
                    ...
                    <div style={{ display: 'flex', gap: 'var(--space-2)' }}>
                        {(details.customerType === 'HOTEL_GUEST' || details.customerType === 'BOTH') && can('bookings:create') && (
                            <Button variant="primary" size="sm" onClick={() => {
                                router.push(`/hotel/bookings?action=new&checkInNow=1&guestId=${encodeURIComponent(details.id)}`);
                            }}>
                                <DoorOpen size={14} style={{ marginRight: '6px' }} /> Check In
                            </Button>
                        )}
                        ...
                        {financials?.stats && financials.stats.balance > 0 && can('finance:record_payment') && (
                            <Button variant="primary" size="sm" onClick={() => setShowPayment(true)}><CreditCard size={14} style={{ marginRight: '6px' }} /> Payment</Button>
                        )}
                        <Button variant="secondary" size="sm" onClick={() => guestId && router.push(`/hotel/finance?tab=customer-ledger&guestId=${encodeURIComponent(guestId)}`)}><FileText size={14} style={{ marginRight: '6px' }} /> Ledger</Button>
                    </div>
                </div>
                ...
                <div style={{ display: 'flex', alignItems: 'center', gap: 'var(--space-4)', padding: 'var(--space-5)', backgroundColor: 'var(--notion-bg-secondary)', borderRadius: 'var(--radius-lg)', border: '1px solid var(--notion-border)', marginBottom: 'var(--space-6)' }}>
                    ...
                    <div style={{ display: 'flex', gap: 'var(--space-4)', textAlign: 'center' }}>
                        ...
                        <div><div style={{ fontSize: '20px', fontWeight: '700', color: (financials?.stats.balance||0)>0?'var(--notion-red)':(financials?.stats.balance||0)<0?'var(--notion-green)':'var(--notion-text)' }}>{(financials?.stats.balance||0)<0?'−':''}NPR {Math.abs(financials?.stats.balance||0).toLocaleString()}</div><div style={{ fontSize: '11px', color: 'var(--notion-text-secondary)', textTransform: 'uppercase' }}>Net Balance</div></div>
                    </div>
                </div>
                ...
                {tab==='ledger' && (
                    <div style={{ animation: 'fadeIn 0.2s ease-out' }}>
                        {financials?.liveBalance != null && financials.liveBalance > 0 && (
                            <div style={{ marginBottom: 'var(--space-4)', padding: '12px 16px', backgroundColor: 'var(--notion-orange-bg)', borderRadius: 'var(--radius-lg)', border: '1px solid var(--notion-orange)', display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
                                <span style={{ fontSize: '13px', fontWeight: '600', color: 'var(--notion-orange)' }}>Live in-house balance</span>
                                <span style={{ fontSize: '16px', fontWeight: '700', color: 'var(--notion-orange)' }}>NPR {financials.liveBalance.toLocaleString()}</span>
                            </div>
                        )}
                        {(!financials||financials.invoices.length===0&&financials.payments.length===0&&financials.orders.length===0&&!(financials.folioCharges?.length))?<EmptyState message="No ledger entries found." icon={<FileText size={32} />} />:<>
                            <div style={{ display:'grid',gridTemplateColumns:'repeat(3,1fr)',gap:'var(--space-4)',marginBottom:'var(--space-4)' }}>
                                ...
                                <div style={{ padding:'16px',backgroundColor:'var(--notion-bg-secondary)',borderRadius:'var(--radius-lg)',border:'1px solid var(--notion-border)',textAlign:'center' }}>
                                    ...
                                    <div style={{ fontSize:'18px',fontWeight:'700',color:financials.stats.balance>0?'var(--notion-red)':financials.stats.balance<0?'var(--notion-green)':'var(--notion-text)' }}>{financials.stats.balance<0?'−':''}NPR {Math.abs(financials.stats.balance).toLocaleString()}</div>
                                </div>
                            </div>
                            <div style={{ backgroundColor:'var(--notion-bg-secondary)',borderRadius:'var(--radius-lg)',border:'1px solid var(--notion-border)',overflow:'hidden' }}>
                                <table style={{ width:'100%',fontSize:'14px',borderCollapse:'collapse' }}>
                                    ...
                                    <tbody>
                                        {(()=>{
                                            const txs:Array<{date:Date;desc:string;debit:number;credit:number;info?:boolean}> = [];
                                            const invoicedBookingIds = new Set(
                                                financials.invoices.filter((i:any)=>!i.isVoided).map((i:any)=>i.bookingId).filter(Boolean)
                                            );
                                            (financials.folioCharges || []).filter((c:any)=>!c.invoiceId).forEach((c:any)=>txs.push({
                                                date: c.date ? new Date(c.date) : new Date(),
                                                desc: c.description || c.category || 'Folio charge',
                                                debit: Number(c.amount||0), credit: 0,
                                            }));
                                            financials.invoices.filter((inv:any)=>!inv.isVoided).forEach((inv:any)=>txs.push({
                                                date:new Date(inv.createdAt),
                                                desc:`Invoice ${inv.invoiceNumber}`,
                                                debit:Number(inv.grandTotal||0), credit:0,
                                            }));
                                            financials.orders.forEach((o:any)=>{
                                                const onInvoice = o.bookingId && invoicedBookingIds.has(o.bookingId);
                                                const amount = onInvoice ? 0 : Number(o.totalAmount || o.subTotal || 0);
                                                txs.push({
                                                    date:new Date(o.createdAt),
                                                    desc:`Order ${o.orderNumber||''} — ${o.orderType?.replace('_',' ')||'Food'}${onInvoice?' (on invoice)':''}`,
                                                    debit:amount, credit:0, info: onInvoice,
                                                });
                                            });
                                            financials.payments.forEach((pay:any)=>txs.push({
                                                date:new Date(pay.createdAt),
                                                desc:`Payment — ${pay.paymentMethod||pay.method||'Cash'}`,
                                                debit:0, credit:Number(pay.amount||0),
                                            }));
                                            txs.sort((a,b)=>b.date.getTime()-a.date.getTime());
                                            return txs.map((tx,i)=>(<tr key={i} style={{borderBottom:'1px solid var(--notion-border)', opacity: tx.info ? 0.65 : 1}}>
                                                ...
                                                <td style={{padding:'10px 16px',textAlign:'right',color:tx.debit?'var(--notion-red)':'var(--notion-text-muted)',fontWeight:tx.debit?500:400}}>{tx.debit?`NPR ${tx.debit.toLocaleString()}`:'—'}</td>
                                                <td style={{padding:'10px 16px',textAlign:'right',color:tx.credit?'var(--notion-green)':'var(--notion-text-muted)',fontWeight:tx.credit?500:400}}>{tx.credit?`NPR ${tx.credit.toLocaleString()}`:'—'}</td>
                                            </tr>));
                                        })()}
                                    </tbody>
                                </table>
