...
type InventoryTab = 'items' | 'movements' | 'suppliers' | 'warehouses' | 'procurement';
...
export default function InventoryPage() {
    ...
    useEffect(() => {
        ...
        if (pathname === '/hotel/procurement') {
            setActiveTab('procurement');
            return;
        }
        const valid: InventoryTab[] = ['items', 'movements', 'suppliers', 'warehouses', 'procurement'];
        if (tabParam && valid.includes(tabParam as InventoryTab)) {
            setActiveTab(tabParam as InventoryTab);
        }
    ...
    return (
        <>
            ...
            {whFormOpen && (
                <div key={editingWarehouse?.id ?? 'new-wh'} style={{ position: 'fixed', inset: 0, background: 'rgba(0,0,0,0.4)', display: 'flex', alignItems: 'center', justifyContent: 'center', zIndex: 100 }} onClick={() => setWhFormOpen(false)}>
            ...
            {/* Vendor Form Modal */}
            {vendorFormOpen && (
                <div key={editingVendor?.id ?? 'new-vendor'} style={{ position: 'fixed', inset: 0, background: 'rgba(0,0,0,0.4)', display: 'flex', alignItems: 'center', justifyContent: 'center', zIndex: 100 }} onClick={() => setVendorFormOpen(false)}>
                    <div style={{ background: 'var(--notion-bg)', border: '1px solid var(--notion-border)', borderRadius: 'var(--radius-lg)', padding: 'var(--space-6)', width: '100%', maxWidth: '420px' }} onClick={e => e.stopPropagation()}>
