...
const STATUS_OPTIONS: PurchaseOrderStatus[] = ['DRAFT', 'APPROVED', 'PARTIALLY_RECEIVED', 'RECEIVED', 'REJECTED', 'CANCELLED'];

const INVENTORY_LINKS = [
    { href: '/hotel/inventory', label: 'Stock Items', icon: Package },
    { href: '/hotel/inventory?tab=movements', label: 'Movements', icon: History },
    { href: '/hotel/inventory?tab=suppliers', label: 'Suppliers', icon: Truck },
    { href: '/hotel/inventory?tab=warehouses', label: 'Warehouses', icon: Warehouse },
] as const;
...
const getStatusStyle = (status: PurchaseOrderStatus) => {
    switch (status) {
        ...
        case 'PARTIALLY_RECEIVED':
            return { bg: 'var(--notion-orange-bg)', text: 'var(--notion-orange)', icon: Truck, label: 'Partial' };
        case 'RECEIVED':
                ...
                <td style={{ padding: '12px 16px' }} onClick={event => event.stopPropagation()}>
                    <div style={{ display: 'flex', gap: '6px', flexWrap: 'wrap' }}>
                        ...
                        {(po.status === 'APPROVED' || po.status === 'PARTIALLY_RECEIVED') && (
                            <Button variant="primary" size="sm" onClick={onReceive}>Receive GRN</Button>
                        )}
                    </div>
                </td>
            </tr>
...
// supplier, or create a new vendor inline ΓÇö so POs link to consistent suppliers.
                ...
                <div style={{
                    ...
                    {query.trim() && !exactMatch && (
                        <div style={{ display: 'flex', flexDirection: 'column' }}>
                            ...
                            >
                                Use ΓÇ£{query.trim()}ΓÇ¥ as supplier
                            </button>
                            <button
                                type="button"
                                disabled={creating}
                            ...
                            >
                                <Plus size={13} /> {creating ? 'AddingΓÇª' : `Add new vendor ΓÇ£${query.trim()}ΓÇ¥`}
                            </button>
                        </div>
                    )}
                    {filtered.length === 0 && !query.trim() && (
                        <div style={{ padding: '12px', fontSize: '13px', color: 'var(--notion-text-secondary)', textAlign: 'center' }}>No vendors yet ΓÇö type a name to add one</div>
                    )}
                </div>
            )}
        </div>
        ...
        <>
                <div className="dashboard-page-pad">
                    ...
                    <div style={{
                        ...
                        <span style={{ fontSize: '12px', fontWeight: 600, color: 'var(--notion-text-secondary)', alignSelf: 'center', marginRight: '4px' }}>
                            Inventory
                        </span>
