...
export default function TablePlanPage() {
    ...
    const [expandedQrs, setExpandedQrs] = useState<Set<number>>(new Set());
    ...
    const getTableQrUrl = (tableId: number) => `nivas://table/${tableId}`;

    const handleQrDownload = (tableId: number, tableNumber: string) => {
        const svg = document.getElementById(`qr-table-${tableId}`)?.querySelector('svg');
        if (!svg) return;
    ...
    const handleQrPrint = (tableId: number, tableNumber: string) => {
        const svg = document.getElementById(`qr-table-${tableId}`)?.querySelector('svg');
        if (!svg) return;
        const serializer = new XMLSerializer();
        const svgStr = serializer.serializeToString(svg);
        ...
        printWindow.document.write(`
            <html>
                ...
                <body style="display:flex;flex-direction:column;align-items:center;justify-content:center;height:100vh;margin:0;font-family:sans-serif;">
                    ...
                    <div style="width:200px;height:200px;">${svgStr}</div>
                    <p style="color:#666;font-size:12px;margin-top:16px;">Scan to open table dashboard</p>
                </body>
            </html>
        `);
                                ...
                                return (
                                    <div key={table.id} style={{
                                        ...
                                        <div style={{ marginBottom: 'var(--space-3)' }}>
                                            ...
                                            >
                                                <QrCode size={14} />
                                                <span style={{ flex: 1, textAlign: 'left' }}>Table QR</span>
                                                <ChevronDown
                                                    size={14}
                                                    style={{
                                                        transition: 'transform 0.2s',
                                                        transform: expandedQrs.has(table.id) ? 'rotate(180deg)' : 'rotate(0deg)'
                                                    }}
                                                />
                                            </button>
                                            {expandedQrs.has(table.id) && (
                                                <div style={{
                                                    marginTop: '8px',
                                                    padding: '12px',
                                                    borderRadius: 'var(--radius-md)',
                                                    ...
                                                    <div id={`qr-table-${table.id}`}>
                                                        <QRCodeSVG
                                                            value={getTableQrUrl(table.id)}
                                                            size={120}
                                                            level="M"
                                                            includeMargin={true}
                                                            bgColor="var(--notion-bg)"
                                                            fgColor="var(--notion-text)"
                                                        />
                                                    </div>
                                                    <div style={{
                                                        display: 'flex',
                                                        ...
                                                        <button
                                                            onClick={() => handleQrDownload(table.id, table.tableNumber)}
                                                            style={{
                                                                display: 'flex',
                                                                alignItems: 'center',
                                                                gap: '4px',
                                                        ...
                                                        <button
                                                            onClick={() => handleQrPrint(table.id, table.tableNumber)}
                                                            style={{
                                                                display: 'flex',
                                                                alignItems: 'center',
                                                                gap: '4px',
