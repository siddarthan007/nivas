...
// Manage Users Modal
function ManageUsersModal({ isOpen, onClose, tenant }: { isOpen: boolean; onClose: () => void; tenant: Tenant | null }) {
    ...
    const fetchUsers = async () => {
        setIsLoading(true);
        try {
            const res = await api.get<User[]>(`/saas-admin/tenants/${tenant!.id}/users`);
            setUsers(res.data || []);
        } catch (err: any) {
...
function TenantCard({
    ...
    useEffect(() => { setLogoError(false); }, [tenant.logoUrl]);
    useEffect(() => {
        let alive = true;
            ...
            <div style={{
                ...
                <div style={{ display: 'flex', alignItems: 'center', justifyContent: "space-between", fontSize: '12px', borderTop: '1px dashed var(--notion-border)', paddingTop: '4px' }}>
                    ...
                    <div style={{ display: 'flex', alignItems: 'center', gap: '8px' }}>
                        {isExpired && (
                            <button onClick={() => onMoreActions('ACTIVATE')} style={{ fontSize: '10px', color: 'var(--notion-green)', background: 'none', border: 'none', cursor: 'pointer', fontWeight: 600 }}>
                                Activate
                            </button>
                        )}
        ...
        try {
            ...
            if (!isEdit && payload.licenseDuration) {
                if (payload.licenseDuration === 'trial') {
                    (payload as any).trialDays = durationMap[payload.licenseDuration] || 14;
                } else {
                    (payload as any).billingCycle = cycleMap[payload.licenseDuration] || 'MONTHLY';
                }
            }
            delete (payload as any).licenseDuration;
            if (!payload.ownerPassword) delete (payload as any).ownerPassword;
            if (!payload.packageId) delete (payload as any).packageId;
            // Convert numeric string fields to numbers; keep decimal fields as strings
            (payload as any).maxRooms = payload.maxRooms ? parseInt(payload.maxRooms, 10) : undefined;
            (payload as any).maxUsers = payload.maxUsers ? parseInt(payload.maxUsers, 10) : undefined;
...
// Activate license without payment (offline / complimentary)
function ActivateLicenseModal({ isOpen, onClose, tenant, onActivate }: {
    isOpen: boolean;
    onClose: () => void;
    tenant: Tenant | null;
    onActivate: (id: string, cycle: 'MONTHLY' | 'ANNUAL') => Promise<{ expiresAt?: string } | null>;
}) {
    ...
    const handleSubmit = async (e: React.FormEvent) => {
        ...
        setIsSubmitting(true);
        try {
            const result = await onActivate(String(tenant.id), cycle);
            if (result?.expiresAt) {
    ...
    return (
        <Modal isOpen={isOpen} onClose={onClose} title={`Activate License: ${tenant?.name}`}>
            {expiresAt ? (
                <div style={{ display: 'flex', flexDirection: 'column', gap: 'var(--space-4)', alignItems: 'center', padding: 'var(--space-4)' }}>
                    <CheckCircle size={48} style={{ color: 'var(--notion-green)' }} />
                    <div style={{ fontSize: '14px', color: 'var(--notion-text-secondary)', textAlign: 'center' }}>
                        License active until <strong style={{ color: 'var(--notion-text)' }}>{new Date(expiresAt).toLocaleDateString()}</strong>
                    </div>
                    <Button type="button" onClick={onClose} style={{ width: '100%' }}>Close</Button>
                </div>
            ) : (
                <form onSubmit={handleSubmit} style={{ display: 'flex', flexDirection: 'column', gap: 'var(--space-4)' }}>
                    <p style={{ fontSize: '13px', color: 'var(--notion-text-secondary)', lineHeight: 1.5, margin: 0 }}>
                        Activate without recording a payment — use when billing was settled offline or as a complimentary renewal.
                    </p>
                    <div>
                        <label style={{ fontSize: '13px', color: 'var(--notion-text-secondary)', marginBottom: '4px', display: 'block' }}>Billing cycle</label>
                        <Select
                            value={cycle}
                            onChange={(e: React.ChangeEvent<HTMLSelectElement>) => setCycle(e.target.value as 'MONTHLY' | 'ANNUAL')}
                            options={[
                    ...
                    <div style={{ display: 'flex', gap: 'var(--space-3)' }}>
                        <Button type="button" variant="secondary" onClick={onClose} style={{ flex: 1 }}>Cancel</Button>
                        <Button type="submit" disabled={isSubmitting} style={{ flex: 1 }}>{isSubmitting ? 'Activating…' : 'Activate'}</Button>
                    </div>
                </form>
            )}
        </Modal>
...
export default function TenantsPage() {
    const {
        ...
        getPaymentHistory, impersonateOwner, activateLicense
    } = useTenants();
    ...
    const [activateModal, setActivateModal] = useState<{ isOpen: boolean, tenant: any }>({ isOpen: false, tenant: null });
    ...
    const handleMoreActions = async (tenant: any, action: string) => {
        ...
        if (action === 'ACTIVATE') setActivateModal({ isOpen: true, tenant });
    ...
    return (
        <>
            ...
            {activateModal.tenant && (
                <ActivateLicenseModal
                    isOpen={activateModal.isOpen}
                    onClose={() => { setActivateModal({ isOpen: false, tenant: null }); fetchTenants(); }}
                    tenant={activateModal.tenant}
