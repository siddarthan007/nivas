...
class RedisRateLimitContext implements Context {
    ...
    async increment(key: string) {
        ...
        if (!redis || redis.status !== 'ready') return this.fallback.increment(key);
        try {
            const rk = `${this.keyPrefix}${key}`;
            const count = await redis.incr(rk);
            if (count === 1) await redis.pexpire(rk, this.duration);
            let ttl = await redis.pttl(rk);
            if (ttl < 0) { await redis.pexpire(rk, this.duration); ttl = this.duration; }
            return { count, nextReset: new Date(Date.now() + ttl) };
        } catch {
...
const clientKey = (req: Request): string => {
    // Trust the provider-set headers first (cf-connecting-ip / x-real-ip are set by
    // the edge, not the client). For X-Forwarded-For take the RIGHTMOST entry —
    // the hop our own proxy appended — not the leftmost, which a client can spoof
    // to a fresh value per request and bypass the limiter.
...
export const rateLimitMiddleware = (app: Elysia) =>
    app.use(rateLimit({
        duration: 60000,
        // A logged-in SPA fires many concurrent queries + polling; 100/min was
        // far too low and tripped under normal use. 600/min per client is safe.
        max: 600,
        generator: clientKey,
        context: new RedisRateLimitContext('rlmain:'),
        errorResponse: 'Rate limit exceeded. Please try again later.'
    }));

// NOTE: keep `scoping: 'scoped'`. elysia-rate-limit defaults to 'global', which
// makes a limiter applied inside any controller throttle the ENTIRE app. This
// auth limiter must only cover the routes it's explicitly attached to.
export const authRateLimitMiddleware = (app: Elysia) =>
    app.use(rateLimit({
        ...
        scoping: 'scoped',
        generator: clientKey,
        context: new RedisRateLimitContext('rlauth:'),
