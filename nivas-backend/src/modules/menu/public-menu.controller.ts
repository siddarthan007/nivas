...
export const publicMenuController = new Elysia({ prefix: '/public' })
    .get('/:slug/menu', async ({ params, set }) => {
        ...
        const hotel = await db.query.hotels.findFirst({
            ...
            columns: { id: true, name: true, logoUrl: true, primaryColor: true, secondaryColor: true, currency: true },
        });
        if (!hotel) {
            set.status = 404;
        ...
        const payload = {
            hotel: { name: hotel.name, logoUrl: hotel.logoUrl, primaryColor: hotel.primaryColor, secondaryColor: hotel.secondaryColor, currency: hotel.currency || 'NPR' },
            categories,
    ...
    .get('/invoice/:id', async ({ params, set }) => {
        const invoice = await db.query.invoices.findFirst({
            ...
            with: { hotel: { columns: { name: true, logoUrl: true, address: true, phone: true, email: true, website: true, panNumber: true, vatNumber: true, primaryColor: true, currency: true } } },
        });
