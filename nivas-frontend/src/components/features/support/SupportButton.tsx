...
// Floating "Need help?" button → panel with platform support contacts.
// Contacts are configured by the SaaS admin and read-only here.
export default function SupportButton({ bottomOffset = 90 }: Props) {
        ...
        <>
            {!open && (
                <button onClick={() => setOpen(true)} aria-label="Get help" title="Help & support"
                    style={{ position: 'fixed', bottom: bottomOffset, right: 24, zIndex: 999, width: 48, height: 48, borderRadius: '50%', border: '1px solid var(--notion-border)', background: 'var(--notion-bg)', color: 'var(--notion-blue)', boxShadow: '0 4px 14px rgba(0,0,0,0.18)', cursor: 'pointer', display: 'flex', alignItems: 'center', justifyContent: 'center', transition: 'transform 120ms' }}
            ...
            {open && (
                <div style={{ position: 'fixed', bottom: bottomOffset, right: 24, zIndex: 999, width: 'min(300px, calc(100vw - 32px))', background: 'var(--notion-bg)', border: '1px solid var(--notion-border)', borderRadius: 14, boxShadow: '0 12px 36px rgba(0,0,0,0.28)', overflow: 'hidden' }}>
