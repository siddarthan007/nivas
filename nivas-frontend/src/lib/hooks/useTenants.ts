...
export function useTenants() {
    ...
    const fetchTenants = useCallback(async () => {
        ...
        try {
            ...
            const mapped: Tenant[] = (response.data || []).map((t: any) => ({
                id: String(t.id),
                name: t.name || '',
                slug: t.slug || '',
                email: t.email || '',
    ...
    const impersonateOwner = async (hotelId: string): Promise<{ success: boolean; token?: string; hotelName?: string }> => {
        try {
            ...
            const response = await api.post<{ token: string; user: { fullName: string; hotelId: number }; hotelName: string }>('/super-admin/impersonate', {
    ...
    // Restore admin session after impersonation - uses secure backend endpoint
    const restoreAdminSession = async (): Promise<boolean> => {
        try {
            // Call backend to get original admin token from HttpOnly cookie
            const response = await api.post<{ restored: boolean; token?: string }>('/super-admin/end-impersonate', {});

            if (response.data?.restored && response.data?.token) {
                // Restore the admin token using proper storage
