        ...
        >
            ...
            <div style={{
                ...
                <div style={{ display: 'flex', gap: '6px' }}>
                    ...
                    {order.status === 'SERVED' && !order.roomId && !order.bookingId && order.paymentStatus !== 'PAID' && onRecordPayment && (
                        <Button
                            size="sm"
        ...
        <Modal isOpen={isOpen} onClose={onClose} title={`Order #${order.orderNumber}`}>
            <div style={{ display: 'flex', flexDirection: 'column', gap: 'var(--space-4)' }}>
                ...
                <div style={{ display: 'flex', gap: 'var(--space-3)', flexWrap: 'wrap' }}>
                    {config.nextStatus && (
                        <Button size="sm" onClick={() => { onStatusChange(order.id, config.nextStatus!); onClose(); }} style={{ flex: 1 }}>
                            <ArrowRight size={14} style={{ marginRight: '4px' }} />
                            {config.nextLabel}
                        </Button>
                    )}
                    {order.status !== 'SERVED' && order.status !== 'CANCELLED' && (
                        <Button size="sm" variant="secondary" onClick={() => { onCancel(order.id); onClose(); }} style={{ color: 'var(--notion-red)' }}>
                            <XCircle size={14} style={{ marginRight: '4px' }} />
                            Cancel Order
                        </Button>
                    )}
                    <Button size="sm" variant="ghost" onClick={async () => { try { await api.post(`/orders/kot/print/${order.id}`); toast.success('KOT sent to printer'); } catch { toast.error('KOT print failed'); } }}>
                        <Printer size={14} style={{ marginRight: '4px' }} />
                        Print KOT
                    </Button>
                    {isOpenOrder && (
                        <Button size="sm" variant="ghost" onClick={doComp} disabled={actionBusy} style={{ color: 'var(--notion-orange)' }}>
                ...
                {/* Pre-bill table ops (open orders only) */}
                {isOpenOrder && (order.orderType === 'DINE_IN') && (
                    <div style={{ display: 'flex', flexDirection: 'column', gap: 'var(--space-2)', borderTop: '1px solid var(--notion-divider)', paddingTop: 'var(--space-3)' }}>
                        <div style={{ display: 'flex', gap: '6px', alignItems: 'center' }}>
                            <select value={moveTo} onChange={e => setMoveTo(e.target.value)} style={{ flex: 1, padding: '8px 10px', fontSize: '13px', border: '1px solid var(--notion-border)', borderRadius: 'var(--radius-md)', background: 'var(--notion-bg)', color: 'var(--notion-text)' }}>
                                <option value="">Move to table…</option>
                                {tables.filter(t => t.id !== order.restaurantTableId).map(t => <option key={t.id} value={t.id}>Table {t.tableNumber}</option>)}
                            </select>
                            <Button size="sm" variant="secondary" onClick={doMove} disabled={actionBusy || !moveTo}>Move</Button>
                        </div>
                        {mergeCandidates.length > 0 && (
                            <div style={{ display: 'flex', gap: '6px', alignItems: 'center' }}>
                                <select value={mergeFrom} onChange={e => setMergeFrom(e.target.value)} style={{ flex: 1, padding: '8px 10px', fontSize: '13px', border: '1px solid var(--notion-border)', borderRadius: 'var(--radius-md)', background: 'var(--notion-bg)', color: 'var(--notion-text)' }}>
                                    <option value="">Merge another order in…</option>
                                    {mergeCandidates.map(o => <option key={o.id} value={o.id}>#{o.orderNumber} ({o.customerName || 'order'})</option>)}
                                </select>
                                <Button size="sm" variant="secondary" onClick={doMerge} disabled={actionBusy || !mergeFrom}>Merge</Button>
                            </div>
                        )}
                    </div>
                )}
                    ...
                    <>
                        ...
                        {stackMode && (
                            <button
                                ...
                                onMouseEnter={e => { const s = e.currentTarget.querySelectorAll('.stack-card'); s.forEach((el, i) => ((el as HTMLElement).style.transform = `translateY(${i * -10}px) rotate(${(i - 1) * 1.5}deg)`)); }}
                                onMouseLeave={e => { const s = e.currentTarget.querySelectorAll('.stack-card'); s.forEach((el, i) => ((el as HTMLElement).style.transform = `translateY(${i * 4}px) scale(${1 - i * 0.04})`)); }}
                            >
                                {[0, 1, 2].map(i => (
                                    <div key={i} className="stack-card" style={{
                                        ...
                                        transition: 'transform 180ms ease', zIndex: 3 - i,
