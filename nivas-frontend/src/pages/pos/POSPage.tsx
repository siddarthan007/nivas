...
export default function POSPage() {
    ...
    const fetchData = useCallback(async () => {
        ...
        try {
            ...
            const methods = payRes.data?.enabledMethods?.length ? payRes.data.enabledMethods : ['CASH', 'CARD', 'FONEPAY'];
    ...
    const placeOrder = async () => {
        ...
        } as CreateOrderPayload & { discountAmount?: number; couponId?: number };

        try {
            const res = await api.post<{ data: { id: string; orderNumber: string } }>('/orders', payload);
        ...
        <div style={{ width: '100vw', height: '100vh', display: 'flex', flexDirection: 'column', background: 'var(--notion-bg-secondary)', overflow: 'hidden' }}>
            ...
            <div style={{ flex: 1, display: 'flex', overflow: 'hidden' }}>
                ...
                <div style={{
                    ...
                    <div style={{ padding: '16px 20px', borderBottom: '1px solid var(--notion-border)', display: 'flex', alignItems: 'center', justifyContent: 'space-between' }}>
                        ...
                        {cart.length > 0 && (
                            <button onClick={clearCart} style={{ background: 'none', border: 'none', cursor: 'pointer', color: 'var(--notion-red)', fontSize: '12px', display: 'flex', alignItems: 'center', gap: '4px' }}>
            ...
            <Modal isOpen={isCheckoutOpen} onClose={() => setIsCheckoutOpen(false)} title="Checkout" size="lg">
                <div style={{ display: 'flex', flexDirection: 'column', gap: 'var(--space-4)', minWidth: '400px' }}>
                    ...
                    {/* Fonepay / wallet QR */}
                    {(paymentMethod === 'FONEPAY' || paymentMethod === 'ESEWA' || paymentMethod === 'KHALTI' || paymentMethod === 'CONNECT_IPS') && (
                ...
                {lastOrder && (
                    <div style={{ display: 'flex', flexDirection: 'column', gap: 'var(--space-3)', minWidth: '280px' }}>
                        <div style={{ textAlign: 'center' }}>
                            <div style={{ fontSize: '16px', fontWeight: 700 }}>NIVAS HOTEL</div>
                        ...
                        <div style={{ fontSize: '12px', color: 'var(--notion-text-secondary)', textAlign: 'center' }}>
                            {lastOrder.orderType.replace('_', ' ')} | {lastOrder.customerName}
                        </div>
                        ...
                        <div style={{ textAlign: 'center', fontSize: '11px', color: 'var(--notion-text-muted)', marginTop: '4px' }}>Thank you for dining with us!</div>
