import { pgTable, serial, text, boolean, timestamp, uuid, integer, pgEnum, json, date, decimal, foreignKey, index, uniqueIndex, varchar } from 'drizzle-orm/pg-core';
...
// Mobile push notification tokens (Expo Push)
export const pushTokens = pgTable('push_tokens', {
    id: serial('id').primaryKey(),
    hotelId: integer('hotel_id').references(() => hotels.id).notNull(),
    userId: uuid('user_id').references(() => (users as any).id).notNull(),
    expoPushToken: text('expo_push_token').notNull(),
    platform: text('platform').notNull(), // 'ios' | 'android'
    deviceId: text('device_id'), // optional hardware ID for dedup
    isActive: boolean('is_active').default(true).notNull(),
    createdAt: timestamp('created_at').defaultNow(),
    updatedAt: timestamp('updated_at').defaultNow(),
}, (table) => [
    index('push_tokens_hotel_idx').on(table.hotelId),
    index('push_tokens_user_idx').on(table.userId),
    uniqueIndex('push_tokens_expo_unique').on(table.expoPushToken),
]);
...
export const mobileEvents = pgTable('mobile_events', {
    ...
    hotelId: integer('hotel_id').references(() => hotels.id).notNull(),
    userId: uuid('user_id').references(() => (users as any).id).notNull(),
    type: text('type').notNull(), // 'screen_view' | 'action' | 'error' | 'session'
    name: text('name').notNull(),
    ...
    index('mobile_events_hotel_idx').on(table.hotelId),
    index('mobile_events_type_idx').on(table.type),
    index('mobile_events_timestamp_idx').on(table.timestamp),
]);
...
export const platformSettings = pgTable('platform_settings', {
    ...
    // Billing contacts shown on SaaS invoices: { name, email, phone, pan, vat, address }.
    billingConfig: json('billing_config').default({}),
    updatedAt: timestamp('updated_at').defaultNow(),
});
