...
type DateRangeOpts = { days?: number; startDate?: string; endDate?: string };

function normalizeDateKey(d: string | Date): string {
    if (typeof d === 'string') return d.split('T')[0] ?? d;
    return d.toISOString().split('T')[0] ?? '';
}

function resolveDateRange(opts: DateRangeOpts) {
...
function fillDailyAmounts(
    start: Date,
    end: Date,
    points: { date: string; amount: number }[],
): { date: string; amount: number }[] {
    const map = new Map(points.map(p => [normalizeDateKey(p.date), p.amount]));
...
async function computeDailyOccupancyFromBookings(hotelId: number, start: Date, end: Date) {
    const [roomsResult] = await db.select({ count: count() }).from(rooms).where(eq(rooms.hotelId, hotelId));
    ...
    const activeBookings = await db.query.bookings.findMany({
        where: and(
            ...
            inArray(bookings.status, ['CHECKED_IN', 'CHECKED_OUT', 'CONFIRMED']),
        ),
        columns: { checkIn: true, checkOut: true, status: true },
    });

    const trend: { date: string; occupancy: number }[] = [];
    const cursor = new Date(start);
    cursor.setHours(0, 0, 0, 0);
    const endDay = new Date(end);
    endDay.setHours(0, 0, 0, 0);
    ...
    while (cursor <= endDay) {
        ...
        for (const b of activeBookings) {
            ...
            if (ci <= dayEnd && co > dayStart) occupied++;
        }
        trend.push({
            date: normalizeDateKey(cursor),
            occupancy: Math.round((occupied / totalRooms) * 1000) / 10,
        });
        cursor.setDate(cursor.getDate() + 1);
    }
    return trend;
...
export const AnalyticsService = {
    async getRealtimeDashboard(hotelId: number) {
        ...
        const [
            ...
            db.select({ value: count() }).from(purchaseOrders)
                .where(and(eq(purchaseOrders.hotelId, hotelId), inArray(purchaseOrders.status, ['DRAFT', 'APPROVED', 'PARTIALLY_RECEIVED']))),
        ]);
    ...
    async getRevenueAnalytics(hotelId: number, opts: DateRangeOpts = {}) {
        const { start, end, days } = resolveDateRange(opts);
        ...
        const [walkInFb] = await db.select({ v: sum(orders.totalAmount) }).from(orders)
            .where(and(eq(orders.hotelId, hotelId), gte(orders.createdAt, start), lte(orders.createdAt, end), eq(orders.status, 'SERVED'), isNull(orders.bookingId)));
        ...
        const rawTrend = dailyRevenue.map(d => ({ date: normalizeDateKey(d.date), amount: parseFloat(d.total ?? '0') }));
        const rawFbTrend = dailyFb.map(d => ({
            date: normalizeDateKey(d.date),
        ...
        return {
            ...
            trend: fillDailyAmounts(start, end, rawTrend),
            fbTrend: fillDailyAmounts(start, end, rawFbTrend).map((p, i) => ({
                ...p,
                orders: rawFbTrend.find(f => f.date === p.date)?.orders ?? 0,
            })),
            comparison: {
    ...
    async getOccupancyAnalytics(hotelId: number, opts: DateRangeOpts = {}) {
        ...
        const occupancyHistory = await db.query.nightAudits.findMany({
            where: and(
                ...
                gte(nightAudits.runAt, start),
                lte(nightAudits.runAt, end),
            ),
        ...
        const auditTrend = occupancyHistory.map(n => ({
            date: normalizeDateKey(n.auditDate),
            occupancy: parseFloat(n.occupancyPercentage ?? '0'),
        }));

        const trend = auditTrend.length > 0
            ? auditTrend.reverse()
            : await computeDailyOccupancyFromBookings(hotelId, start, end);

        const avgOccupancy = trend.length > 0
            ? trend.reduce((sum, n) => sum + n.occupancy, 0) / trend.length
            : 0;
        ...
        return {
            ...
            currentOccupancy: Object.fromEntries(
                roomTypeBreakdown.map(r => [r.type ?? 'UNKNOWN', {
                    total: r.total,
                    occupied: Number(r.occupied ?? 0),
    ...
    async getKeyMetrics(hotelId: number, opts: DateRangeOpts = {}) {
        const { start, end, days } = resolveDateRange(opts);
        ...
        let roomsSold = auditLogs.length > 0
        ...
        if (roomsSold <= 0) {
            ...
            for (const b of overlapping) {
                const ci = new Date(Math.max(new Date(b.checkIn).getTime(), start.getTime()));
                const co = new Date(Math.min(new Date(b.checkOut).getTime(), end.getTime()));
                roomsSold += Math.max(1, Math.ceil((co.getTime() - ci.getTime()) / (1000 * 60 * 60 * 24)));
            }
        }
