...
export const bookingsController = new Elysia({ prefix: '/bookings' })
    ...
    .post('/group', async ({ body, user, request }) => {
        ...
        body: t.Object({
            ...
            guestName: s.string({ minLength: 1 }),
            guestPhone: s.string({ minLength: 5 }),
            guestEmail: t.Optional(t.String()),
    ...
    .post('/walk-in-check-in', async ({ body, user, request }) => {
        if (!user?.hotelId) throw new ValidationError('Hotel ID is required');
        const ip = request.headers.get('x-forwarded-for') || undefined;
        const result = await BookingsService.walkInCheckIn(user.hotelId, user.id, body, ip);
        return createResponse(result, 'Walk-in guest checked in successfully');
    }, {
        isSignedIn: true,
        hasPermission: PERMISSIONS.BOOKINGS.CREATE,
        body: t.Object({
        ...
        detail: { summary: 'Walk-in check-in (create booking + check in)', tags: ['Bookings'] },
    })
