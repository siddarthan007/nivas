...
export const CouponsService = {
    ...
    async validate(hotelId: number, code: string, amount: number, scope: 'ROOM' | 'FNB' | 'ALL' = 'ALL') {
        ...
        if (!coupon) throw new NotFoundError('Coupon');
        return this.validateCouponRow(coupon, amount, scope);
    },

    async validateById(hotelId: number, couponId: number, amount: number, scope: 'ROOM' | 'FNB' | 'ALL' = 'ALL') {
        const coupon = await db.query.coupons.findFirst({
            where: and(eq(coupons.hotelId, hotelId), eq(coupons.id, couponId)),
        });
        if (!coupon) throw new NotFoundError('Coupon');
        return this.validateCouponRow(coupon, amount, scope);
    },
