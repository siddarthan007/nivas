...
export const guestsController = new Elysia({ prefix: '/crm/guests' })
    ...
    .get('/:id/timeline', async ({ params, user }) => {
        if (!user?.hotelId) throw new ValidationError('Hotel ID is required');
        const data = await GuestService.getGuestTimeline(user.hotelId, params.id);
        return createResponse(data, 'Guest timeline fetched successfully');
    }, {
        isSignedIn: true,
        hasPermission: PERMISSIONS.GUESTS.VIEW_DETAILS,
        detail: { summary: 'Get guest timeline (messages, complaints, stays)', tags: ['CRM'] }
    })
