...
export const engineController = new Elysia({ prefix: '/engine' })
    ...
    .get('/availability', async ({ api, query, set }) => {
        const inD = parseBookingDate(query.checkIn, 'checkIn');
        const outD = parseBookingDate(query.checkOut, 'checkOut');
        if (outD <= inD) {
    ...
    }, { body: t.Object({ contact: s.string({ minLength: 5 }), channel: t.Optional(t.Union([t.Literal('email'), t.Literal('sms')])) }) })
    ...
    .post('/bookings', async ({ api, body, set }) => {
        ...
        const inD = parseBookingDate(body.checkIn, 'checkIn');
        const outD = parseBookingDate(body.checkOut, 'checkOut');
        if (outD <= inD) { set.status = 400; return createResponse(null, 'Invalid dates'); }
        ...
        body: t.Object({
            ...
            guestName: s.string({ minLength: 1, maxLength: 120 }),
            guestPhone: s.string({ minLength: 5, maxLength: 20 }),
            guestEmail: t.Optional(s.string({ maxLength: 160 })),
            otp: s.string({ minLength: 4, maxLength: 8 }),
        }),
    });
