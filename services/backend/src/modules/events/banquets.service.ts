...
export const BanquetsService = {
    ...
    async generateBookingInvoice(hotelId: number, userId: string, bookingId: string) {
        const booking = await this.getBookingById(hotelId, bookingId);
        if (booking.invoiceId) throw new BusinessLogicError('Invoice already exists for this event');
        if (booking.status !== 'CONFIRMED' && booking.status !== 'COMPLETED') {
            throw new BusinessLogicError('Invoice can only be generated for confirmed or completed events');
        }

        const subTotal = parseFloat(booking.totalAmount || '0');
        if (subTotal <= 0) throw new BusinessLogicError('Set the event total amount before generating an invoice');

        const guestName = relationOne(booking.guest)?.fullName || booking.organizerName || 'Event Guest';
        const venueName = (booking as any).banquet?.name || 'Venue';
        const description = `${booking.eventName} — ${venueName}`;

        await db.transaction(async (tx) => {
            const { invoice } = await InvoiceService.generateEventInvoice(
                hotelId,
                userId,
                { guestName, subTotal, description },
                tx
            );
            await tx.update(banquetBookings)
        ...
        await AuditService.log(
            hotelId,
            userId,
            'GENERATE_BANQUET_INVOICE',
            'BANQUET_BOOKING',
            bookingId,
            { eventName: booking.eventName }
        );

        return await this.getBookingById(hotelId, bookingId);
    },
