...
import { syncOrderPaymentStatus } from '../orders/order-payment.util';
        ...
        const newPayment = await db.transaction(async (tx) => {
            ...
            if (data.orderId) {
                ...
                if (order?.bookingId) {
                    ...
                    // Walk-in order payment: cash sale — lodge under room/folio revenue for hotel guests
                    const revAccount = await GLService.getOrCreateControlAccount(hotelId, '4000', 'Room Revenue', 'REVENUE', tx);
                    lines = [
                        { accountId: debitAccount!.id, debit: data.amount, credit: 0, description: `Order payment via ${data.paymentMethod}` },
                        { accountId: revAccount!.id, debit: 0, credit: data.amount, description: 'Lodging Revenue' }
                    ];
                }
            } else {
            ...
            if (data.orderId) {
                ...
                await syncOrderPaymentStatus(tx, hotelId, data.orderId);
        ...
        if (newPayment) {
            ...
            if (data.bookingId) {
                const booking = await db.query.bookings.findFirst({
                    where: and(eq(bookings.id, data.bookingId), eq(bookings.hotelId, hotelId)),
                    columns: { roomId: true, status: true },
                });
    ...
    static async getPayments(hotelId: number, limit: number = 50) {
        return await db.query.payments.findMany({
            ...
            with: {
                ...
                booking: {
                    columns: { guestId: true, guestName: true, guestPhone: true }
                }
            },
