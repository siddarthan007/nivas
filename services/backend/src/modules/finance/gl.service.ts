...
export const GLService = {
     ...
     */
    async getOrCreateControlAccount(hotelId: number, code: string, name: string, type: 'ASSET' | 'LIABILITY' | 'EQUITY' | 'REVENUE' | 'EXPENSE', dbTx?: DbTransaction, isControl: boolean = true) {
    ...
    async initializeChartOfAccounts(hotelId: number) {
        ...
        for (const acc of standardAccounts) {
            await this.getOrCreateControlAccount(
                hotelId,
                acc.code,
                acc.name,
                acc.type as 'ASSET' | 'LIABILITY' | 'EQUITY' | 'REVENUE' | 'EXPENSE',
                undefined,
                acc.isControl ?? false,
            );
        }
     ...
     */
    async postJournalEntry(hotelId: number, userId: string | null, date: string, description: string, reference: string, lines: JournalLineInput[], dbTx?: DbTransaction) {
    ...
    async reverseJournalEntry(hotelId: number, userId: string, entryId: string, reason: string) {
        return db.transaction(async (tx) => {
            ...
            const r2 = (v: string) => Math.round((parseFloat(v) || 0) * 100) / 100;
    ...
    async getTrialBalance(hotelId: number, asOfDate: string) {
        ...
        const res = await db.execute(query) as SqlResult;
        const rows: TrialBalanceRow[] = Array.isArray(res) ? res as unknown as TrialBalanceRow[] : (res as { rows: unknown[] })?.rows as unknown as TrialBalanceRow[] ?? [];
        return rows;
    },
    ...
    async getProfitAndLoss(hotelId: number, fromDate: string, toDate: string) {
        ...
        const list: ProfitLossRow[] = Array.isArray(result) ? result as unknown as ProfitLossRow[] : (result as { rows: unknown[] })?.rows as unknown as ProfitLossRow[] ?? [];
    ...
    async getBalanceSheet(hotelId: number, asOfDate: string) {
        ...
        const list: BalanceSheetRow[] = Array.isArray(result) ? result as unknown as BalanceSheetRow[] : (result as { rows: unknown[] })?.rows as unknown as BalanceSheetRow[] ?? [];
    ...
    async getAccountLedger(hotelId: number, accountId: number, fromDate: string, toDate: string) {
        ...
        `);
        const openRows = Array.isArray(openRes) ? openRes : (openRes as { rows: unknown[] })?.rows ?? [];
        ...
        `);
        const list: LedgerLine[] = Array.isArray(linesRes) ? linesRes as unknown as LedgerLine[] : (linesRes as { rows: unknown[] })?.rows as unknown as LedgerLine[] ?? [];
    ...
    async getCashFlow(hotelId: number, fromDate: string, toDate: string) {
        ...
        `);
        const daily: CashFlowDaily[] = Array.isArray(dailyRes) ? dailyRes as unknown as CashFlowDaily[] : (dailyRes as { rows: unknown[] })?.rows as unknown as CashFlowDaily[] ?? [];
    ...
    async getCashTransactions(hotelId: number, fromDate: string, toDate: string, limit = 300) {
        ...
        const res = await db.execute(sql`
            SELECT je.id AS id, je.date AS date, je.description AS description, je.reference AS reference,
                   ...
                   COALESCE(SUM(jl.credit), 0) AS outflow,
                   CASE
                     WHEN p.booking_id IS NOT NULL OR o.booking_id IS NOT NULL THEN 'lodging'
                     WHEN o.id IS NOT NULL OR je.description ILIKE '%order%' OR je.description ILIKE '%folio%' OR je.description ILIKE '%room%' THEN 'lodging'
                     WHEN je.description ILIKE '%payment%' OR je.description ILIKE '%invoice%' THEN 'lodging'
                     ELSE 'lodging'
                   END AS category
            FROM journal_lines jl
            JOIN accounts a ON a.id = jl.account_id
            JOIN journal_entries je ON je.id = jl.journal_entry_id
            LEFT JOIN payments p ON p.id::text = je.reference
            LEFT JOIN orders o ON o.id = p.order_id
            WHERE ${cashFilter} AND je.status = 'POSTED' AND je.date >= ${fromDate} AND je.date <= ${toDate}
            GROUP BY je.id, je.date, je.description, je.reference, je.created_at, p.booking_id, o.booking_id, o.id
            ORDER BY je.date DESC, je.created_at DESC
            LIMIT ${limit}
        `) as { rows?: Record<string, unknown>[] } | Record<string, unknown>[];
        const rows: Record<string, unknown>[] = Array.isArray(res) ? res : (res.rows ?? []);
        return rows.map(r => {
            ...
            return {
                ...
                category: (r.category as string) || 'lodging',
            };
        });
    },
    ...
    async getArAging(hotelId: number, asOfDate: string) {
        ...
        for (const inv of rows) {
            ...
            const age = Math.max(0, Math.floor((asOf - new Date(inv.createdAt || new Date()).getTime()) / dayMs));
        ...
        const customers = Array.from(byCustomer.values()).map(rnd).sort((a: any, b: any) => (b.total || 0) - (a.total || 0));
