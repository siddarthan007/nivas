...
export const InvoiceService = {
     ...
     */
    async getNextInvoiceNumber(hotelId: number, tx: DbTransaction = db): Promise<{ number: string; sequence: number; fiscalYear: string }> {
     ...
     */
    async generateInvoice(hotelId: number, userId: string, data: { bookingId: string; discount?: number; guestPan?: string; doCheckout?: boolean }, dbTx?: DbTransaction) {
        ...
        const core = async (tx: DbTransaction) => {
    ...
    /**
     * Generate an invoice for banquet/event charges (no room booking).
     */
    async generateEventInvoice(
        hotelId: number,
        userId: string,
        data: { guestName: string; guestPan?: string; subTotal: number; description: string },
        dbTx?: DbTransaction
    ) {
        const core = async (tx: DbTransaction) => {
            await tx.execute(sql`SELECT pg_advisory_xact_lock(${hotelId})`);
            ...
            const hotel = await tx.query.hotels.findFirst({
                where: eq(hotels.id, hotelId),
                columns: { currency: true, serviceChargeRate: true, taxRate: true },
            });
            const currency = hotel?.currency ?? 'NPR';
            ...
            if (!inv) throw new Error('Failed to create event invoice');
            ...
            const lines = [
                { accountId: arAccount!.id, debit: grandTotal, credit: 0, description: `Event invoice ${number}` },
                { accountId: fbRevAccount!.id, debit: 0, credit: subTotal, description: data.description },
            ];
            if (serviceCharge > 0) {
                lines.push({ accountId: fbRevAccount!.id, debit: 0, credit: serviceCharge, description: 'Service charge' });
            }
            if (vat > 0) {
                lines.push({ accountId: taxAccount!.id, debit: 0, credit: vat, description: 'VAT' });
            }
            ...
            await GLService.postJournalEntry(
                ...
                userId,
                new Date().toISOString().split('T')[0] as string,
                `Event invoice ${number} — ${data.guestName}`,
                number,
                lines,
                tx
            );
        ...
        const invoice = dbTx ? await core(dbTx) : await db.transaction(async (tx) => core(tx));
        return { invoice, grandTotal: parseFloat(invoice.grandTotal) };
    },
    ...
    async getInvoiceData(invoiceId: string, hotelId?: number) {
        ...
        // Get folio charges (room bookings only)
        const charges = invoice.bookingId
            ? await db.query.folioCharges.findMany({
        ...
        const roomOrders = invoice.bookingId
        ...
        // Build line items — skip order-linked folio charges (shown via order items instead)
        const chargedOrderIds = new Set(
            charges.filter(c => c.orderId).map(c => c.orderId as string)
        );
        const nonOrderCharges = charges.filter(c => !c.orderId);
        const billableOrders = roomOrders.filter(o => !chargedOrderIds.has(o.id));

        const lineItems: Array<{ description: string; quantity: number; rate: number; amount: number }> = [
            ...nonOrderCharges.map(c => ({
                description: c.description,
                quantity: 1,
                rate: parseFloat(c.amount ?? '0'),
                amount: parseFloat(c.amount ?? '0')
            })),
            ...billableOrders.flatMap(o => o.items.map(i => ({
        ...
        // If no line items (event invoice), add a single summary line
        if (lineItems.length === 0 && subTotal > 0) {
            lineItems.push({
                description: invoice.guestName ? `Event — ${invoice.guestName}` : 'Event charges',
                quantity: 1,
                rate: subTotal,
                amount: subTotal,
            });
        ...
        if (nonOrderCharges.length === 0 && booking) {
        ...
        const hotelBranding = {
            ...
            serviceChargeRate: hotel.serviceChargeRate ?? '0.10',
            vatRate: hotel.taxRate ?? '0.13',
    ...
    async generatePdf(invoiceId: string, hotelId?: number): Promise<Buffer> {
        ...
        const logoDataUrl = data.hotel?.logoUrl
            ? await PdfService.fetchImageAsDataUrl(data.hotel.logoUrl)
            : null;
        const docDefinition = PdfService.generateInvoiceDefinition({
            ...data,
