...
export const GuestActionsService = {
    async validateGuestAccess(user: any) {
        ...
        await GuestAuthService.assertActiveStay(user.hotelId!, user.roomId);
    ...
    async placeOrder(hotelId: number, roomId: number, userId: string, items: { menuItemId: number; quantity: number; notes?: string }[], deliveryTo?: 'ROOM' | 'RESTAURANT', orderNotes?: string) {
        ...
        if (!newOrder) throw new BusinessLogicError('Failed to place order');
    ...
    async getPortalConfig(hotelId: number, roomId?: number) {
        ...
        if (roomId) {
            const room = await db.query.rooms.findFirst({
                where: eq(rooms.id, roomId),
                columns: { dndStatus: true },
            });
            dndEnabled = !!room?.dndStatus;
        }
        return {
