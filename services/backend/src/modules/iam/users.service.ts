...
export class UsersService {
    static async getStaff(hotelId: number) {
        ...
        return staffList.map(({ passwordHash, ...u }) => ({
            ...u,
            hasMobileApp: (u.pushTokens || []).some((t: any) => t.isActive),
            mobileDevices: (u.pushTokens || []).filter((t: any) => t.isActive).map((t: any) => ({
                platform: t.platform,
                deviceId: t.deviceId,
    ...
    static async deleteUser(hotelId: number, requesterId: string, userId: string) {
        // Cannot delete yourself
        if (requesterId === userId) throw new ForbiddenError('You cannot delete your own account');

        // Hierarchy: cannot delete someone who outranks you
        const [requester, target] = await Promise.all([
            db.query.users.findFirst({ where: and(eq(users.id, requesterId), eq(users.hotelId, hotelId)), with: { role: true } }),
            db.query.users.findFirst({ where: and(eq(users.id, userId), eq(users.hotelId, hotelId)), with: { role: true } }),
        ]);
        ...
        if (!isSuperAdmin) {
            const reqLevel = (requester as any)?.role?.level ?? Infinity;
            if (((target as any)?.role?.level ?? Infinity) < reqLevel) throw new ForbiddenError('You cannot delete a user above your level');
        }
        ...
        const owners = active.filter(u => (u as any).role?.level === 0);
        const targetIsOwner = owners.some(u => u.id === userId);
        if (targetIsOwner && owners.length <= 1) throw new ForbiddenError('Cannot delete the last active owner');

        // Soft delete: mark inactive, clear email, and append deleted timestamp
        const [updatedUser] = await db.update(users)
            .set({
                isActive: false,
                email: sql`CONCAT(${users.email}, '.deleted.', EXTRACT(EPOCH FROM NOW())::int)`,
                updatedAt: new Date()
            })
            .where(and(
                eq(users.id, userId),
