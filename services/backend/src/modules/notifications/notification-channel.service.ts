...
import { applyMessageTemplate, renderTemplateHtml } from '../../utils/message-template.util';
import {
    mergeMessageChannelPrefs,
    isChannelEnabled,
    type MessageEventId,
...
export const NotificationChannelService = {
    ...
    async getChannelPrefs(hotelId: number) {
        const hotel = await db.query.hotels.findFirst({
            where: eq(hotels.id, hotelId),
            columns: { notificationConfig: true },
        });
        const cfg = (hotel?.notificationConfig || {}) as Record<string, any>;
        return mergeMessageChannelPrefs(cfg.messageChannels);
    },

    async getEmailTemplate(hotelId: number, key: string): Promise<string | null> {
        const hotel = await db.query.hotels.findFirst({
            where: eq(hotels.id, hotelId),
            columns: { notificationConfig: true },
        });
        const tpl = (hotel?.notificationConfig as any)?.emailTemplates?.[key];
        return typeof tpl === 'string' && tpl.trim() ? tpl : null;
    },
    ...
    async send(
        ...
        const prefs = opts?.event ? await this.getChannelPrefs(hotelId) : null;
        const event = opts?.event;

        const results: { channels: Array<{ type: string; success: boolean; provider?: string; error?: string }> } = { channels: [] };

        const allowSms = (!event || !prefs || isChannelEnabled(prefs, event, 'sms'))
            && features?.enableSmsNotifications
            && settings?.smsProvider
            && settings?.smsApiKey
            && !!recipientPhone?.trim();
        const allowWhatsapp = (!event || !prefs || isChannelEnabled(prefs, event, 'whatsapp'))
            && features?.enableWhatsappNotifications
            && settings?.whatsappApiKey
            && settings?.whatsappProvider
            && !!recipientPhone?.trim();
        const allowEmail = (!event || !prefs || isChannelEnabled(prefs, event, 'email'))
            && features?.enableEmailNotifications
            && settings?.smtpHost
        ...
        // Send WhatsApp if enabled and configured
        if (allowWhatsapp) {
    ...
    async sendSms(settings: any, phone: string, message: string) {
        if (!phone?.trim()) return { success: false, error: 'No recipient phone' };
    ...
    async sendBrandedEmail(hotelId: number, to: string, subject: string, content: EmailContent) {
        ...
        if (!settings?.smtpHost || !settings?.smtpPassword) return { success: false, error: 'email not configured' };
    ...
    async sendEmail(settings: any, to: string, subject: string, htmlBody: string) {
        ...
        if (!settings?.smtpHost || !settings?.smtpPassword) {
    ...
    async sendBookingConfirmation(hotelId: number, guestPhone: string, guestEmail: string | undefined, bookingDetails: any) {
        const settings = await this.resolveSettings(hotelId);
        const vars = {
            guestName: String(bookingDetails.guestName || 'Guest'),
            hotelName: String(bookingDetails.hotelName || 'Hotel'),
            checkIn: String(bookingDetails.checkIn || ''),
            checkOut: String(bookingDetails.checkOut || ''),
            roomNumber: String(bookingDetails.roomNumber || ''),
            bookingId: String(bookingDetails.bookingId || ''),
        };
        const sms = applyMessageTemplate(
            settings.bookingConfirmationTemplate,
            vars,
            `Your booking at ${vars.hotelName} is confirmed! Check-in: ${vars.checkIn}, Room: ${vars.roomNumber}, Confirmation: ${vars.bookingId}`,
        );
        const r = await this.send(hotelId, guestPhone, undefined, sms, 'booking_confirmation', { event: 'bookingConfirmation' });
        if (guestEmail) {
            const prefs = await this.getChannelPrefs(hotelId);
            if (isChannelEnabled(prefs, 'bookingConfirmation', 'email')) {
                const customHtml = await this.getEmailTemplate(hotelId, 'bookingConfirmation');
                if (customHtml) {
                    const [settings, hotel] = await Promise.all([
                        this.resolveSettings(hotelId),
                        db.query.hotels.findFirst({
                            where: eq(hotels.id, hotelId),
                            columns: { name: true, logoUrl: true, primaryColor: true, website: true, phone: true, email: true, address: true },
                        }),
                    ]);
                    const bodyHtml = renderTemplateHtml(customHtml, vars, '');
                    const html = renderBrandedEmail(hotel || {}, {
                        heading: 'Your booking is confirmed',
                        intro: bodyHtml.replace(/<[^>]+>/g, ' ').trim() || undefined,
                        rows: [
                            { label: 'Confirmation', value: vars.bookingId },
                            { label: 'Check-in', value: vars.checkIn },
                            { label: 'Room', value: vars.roomNumber },
                        ],
                        footerNote: 'We look forward to hosting you.',
                    });
                    await this.sendEmail(settings, guestEmail, 'Booking Confirmed', html);
                } else {
                    await this.sendBrandedEmail(hotelId, guestEmail, 'Booking Confirmed', {
                        heading: 'Your booking is confirmed',
                        rows: [
                            { label: 'Confirmation', value: vars.bookingId },
                            { label: 'Check-in', value: vars.checkIn },
    ...
    async sendCheckInReminder(hotelId: number, guestPhone: string, guestEmail: string | undefined, bookingDetails: any) {
        ...
        const vars = {
            ...
            hotelName: String(bookingDetails.hotelName || 'Hotel'),
            roomNumber: String(bookingDetails.roomNumber || ''),
            checkInTime: String(bookingDetails.checkInTime || ''),
        };
        const sms = applyMessageTemplate(
            settings.checkInReminderTemplate,
            vars,
            `Reminder: Your check-in at ${vars.hotelName} is tomorrow! Room: ${vars.roomNumber}, Time: ${vars.checkInTime}`,
        );
        const r = await this.send(hotelId, guestPhone, undefined, sms, 'checkin_reminder', { event: 'checkInReminder' });
        if (guestEmail) {
            const prefs = await this.getChannelPrefs(hotelId);
            if (isChannelEnabled(prefs, 'checkInReminder', 'email')) {
                await this.sendBrandedEmail(hotelId, guestEmail, 'Check-in Reminder', {
    ...
    async sendPaymentReceipt(hotelId: number, guestPhone: string, guestEmail: string | undefined, paymentDetails: any) {
        const settings = await this.resolveSettings(hotelId);
        const vars = {
            guestName: String(paymentDetails.guestName || 'Guest'),
            amount: String(paymentDetails.amount || ''),
            currency: String(paymentDetails.currency || 'NPR'),
            invoiceNumber: String(paymentDetails.invoiceNumber || ''),
            hotelName: String(paymentDetails.hotelName || 'Hotel'),
        };
        const sms = applyMessageTemplate(
            settings.paymentReceiptTemplate,
            vars,
            `Payment received: ${vars.currency} ${vars.amount}. Invoice: ${vars.invoiceNumber}. Thank you!`,
        );
        const r = await this.send(hotelId, guestPhone, undefined, sms, 'payment_receipt', { event: 'paymentReceipt' });
        if (guestEmail) {
            const prefs = await this.getChannelPrefs(hotelId);
            if (isChannelEnabled(prefs, 'paymentReceipt', 'email')) {
                await this.sendBrandedEmail(hotelId, guestEmail, 'Payment Receipt', {
    ...
    async sendCheckoutNotification(hotelId: number, guestPhone: string, guestEmail: string | undefined, details: any) {
        ...
        const r = await this.send(hotelId, guestPhone, undefined, sms, 'checkout_notification', { event: 'checkout' });
        if (guestEmail) {
            const prefs = await this.getChannelPrefs(hotelId);
            if (isChannelEnabled(prefs, 'checkout', 'email')) {
                await this.sendBrandedEmail(hotelId, guestEmail, 'Thank you for your stay', {
