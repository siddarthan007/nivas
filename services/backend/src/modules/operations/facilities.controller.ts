...
export const facilitiesController = new Elysia({ prefix: '/operations/facilities' })
    ...
    .get('/parking/assignable-bookings', async ({ user }) => {
        if (!user?.hotelId) throw new ValidationError('Hotel ID is required');
        const list = await ParkingService.getAssignableBookings(user.hotelId);
        return createResponse(list, 'Assignable in-house bookings');
    }, {
        isSignedIn: true,
        hasPermission: PERMISSIONS.PARKING.MANAGE,
        detail: { summary: 'In-house bookings for parking assignment', tags: ['Operations'] },
    })
