...
export async function syncTableStatus(tx: Tx, hotelId: number, tableId?: number | null) {
    ...
    const liveOrders = await tx.query.orders.findMany({
        ...
        columns: { id: true, totalAmount: true, status: true, paymentStatus: true },
    });

    let busy = false;
    for (const o of liveOrders) {
        // Fully paid and served — table can be freed even if order row still exists
        if (o.status === 'SERVED' && (o as { paymentStatus?: string }).paymentStatus === 'PAID') {
            continue;
        }
