...
export const tablesController = new Elysia({ prefix: '/operations/tables' })
    ...
    .get('/', async ({ user }) => {
        ...
        isSignedIn: true,
        hasPermission: PERMISSIONS.RESTAURANT.VIEW_TABLES,
        detail: { summary: 'List all tables', tags: ['Operations'] }
    })
    .get('/qrcodes', async ({ user }) => {
        if (!user?.hotelId) throw new ValidationError('Hotel ID is required');
        const list = await TablesService.getTableQrCodes(user.hotelId);
        return createResponse(list, 'Table QR codes fetched successfully');
    }, {
        isSignedIn: true,
        hasPermission: PERMISSIONS.RESTAURANT.VIEW_TABLES,
        detail: { summary: 'List table QR payloads', tags: ['Operations'] }
    })
