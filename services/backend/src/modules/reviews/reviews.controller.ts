...
export const reviewsController = new Elysia({ prefix: '/reviews' })
    ...
    .post('/', async ({ user, body }) => {
        ...
        body: t.Object({
            ...
            comment: t.Optional(s.string({ maxLength: 4000 })),
    ...
    .post('/:id/publish-reply', async ({ user, params, body }) => {
        if (!user?.hotelId) throw new ValidationError('Hotel ID is required');
        const saved = await ReviewsService.saveReply(user.hotelId, parseInt(params.id), body.replyText, true);
        return createResponse(saved, 'Reply published');
    }, {
        isSignedIn: true,
        hasPermission: PERMISSIONS.ANALYTICS.VIEW_OPERATIONS,
        params: t.Object({ id: t.String() }),
        body: t.Object({ replyText: s.string({ minLength: 1 }) }),
        detail: { summary: 'Publish review reply', tags: ['Reviews'] }
    });
