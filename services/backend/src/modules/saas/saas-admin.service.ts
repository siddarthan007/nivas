...
export const SaasAdminService = {
    ...
    async getSupportConfig() {
        const ps = await db.query.platformSettings.findFirst({ where: eq(platformSettings.id, 1), columns: { supportConfig: true } });
        const c = (ps?.supportConfig || {}) as SupportConfig;
    ...
    async setSupportConfig(data: { email?: string; phone?: string; whatsapp?: string; hours?: string }) {
        const ps = await db.query.platformSettings.findFirst({ where: eq(platformSettings.id, 1), columns: { supportConfig: true } });
        const next = { ...((ps?.supportConfig || {}) as SupportConfig) };
        for (const k of ['email', 'phone', 'whatsapp', 'hours'] as const) if (data[k] !== undefined) next[k] = data[k];
        await db.insert(platformSettings).values({ id: 1, supportConfig: next } as { id: number; supportConfig: SupportConfig })
    ...
    async getDatabaseStats() {
        const rows = await db.execute(sql`
            ...
            LIMIT 25`);
        const arr = ((rows as unknown as { rows?: DatabaseTableRow[] })?.rows ?? (rows as unknown as DatabaseTableRow[])) || [];
        ...
        const tables = await Promise.all(arr.map(async (r) => {
            let rowCount = 0;
            try {
                const c = await db.execute(sql.raw(`SELECT count(*)::int AS n FROM "${String(r.table).replace(/"/g, '')}"`));
                rowCount = Number(((c as unknown as { rows?: { n: number }[] })?.rows ?? (c as unknown as { n: number }[]))[0]?.n || 0);
    ...
    async getAllTenants() {
        ...
        return allHotels.map(hotel => {
            ...
            if (!owner) {
                console.warn(`[SaasAdmin] Hotel ${hotel.id} (${hotel.name}) has no Owner role user`);
            }
    ...
    async onboardTenant(data: OnboardTenantData, createdById: string) {
    ...
    async recordSaaSPayment(hotelId: number, data: SaaSPaymentData, createdById: string) {
    ...
    async getTenantDetails(hotelId: number) {
        try {
            ...
            throw new HttpError(`Failed to load tenant details: ${(err as Error).message}`, 500, 'TENANT_DETAILS_ERROR');
        }
    },

    async updateTenantDetails(hotelId: number, data: UpdateTenantData) {
        const allowed = ['name', 'slug', 'email', 'phone', 'address', 'website', 'logoUrl', 'panNumber', 'vatNumber', 'currency', 'checkInTime', 'checkOutTime', 'taxRate', 'serviceChargeRate', 'isActive', 'planTier', 'maxRooms', 'maxUsers'] as const;
        ...
        const hotelUpdate: Record<string, any> = { ...updateData };
        if (updateData.serviceChargeRate !== undefined) {
            const v = updateData.serviceChargeRate;
            hotelUpdate.serviceChargeRate = (typeof v === 'string' && v !== '' ? parseFloat(v) / 100 : (v ?? 0.10)).toString();
        }
        if (updateData.taxRate !== undefined) {
            const v = updateData.taxRate;
            hotelUpdate.taxRate = (typeof v === 'string' && v !== '' ? parseFloat(v) / 100 : (v ?? 0.13)).toString();
        }
    ...
    async updatePackage(id: number, data: UpdatePackageData) {
        ...
        const flags = buildTenantFeatureFlags(updated);
    ...
    getAvailableModules() {
        return [
            ...
            { id: 'events', label: 'Events & Banquets', category: 'Venues' },
            { id: 'venues', label: 'Venue Management', category: 'Venues' },
