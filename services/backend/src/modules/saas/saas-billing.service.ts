...
export const SaasBillingService = {
    ...
    async getMySubscription(hotelId: number) {
        ...
        const pkg = relationOne(subscription?.package);
        const rawFeatures = (pkg?.features as string[]) || [];
    ...
    async getBillingStats() {
        ...
        return {
            currentPlan: relationOne(activeSubscription?.package)?.name || 'No active plan',
    ...
    async subscribe(hotelId: number, userId: string, packageId: number, billingCycle: 'MONTHLY' | 'ANNUAL' | '2_YEAR' | '3_YEAR', ip?: string) {
        ...
        // Sync tenant features immediately when a plan is selected (AI, CBMS, venues, modules).
        await LicenseService.applyPackageFeatures(hotelId, packageId);
    ...
    async generateInvoicePdf(paymentId: string) {
        ...
        const data = {
            ...
            taxRate,
            serviceChargeRate,
            packageName: relationOne(subscription?.package)?.name || 'Subscription',
        ...
        try {
            const docDefinition = await PdfService.generateSaasInvoiceDefinition(data);
