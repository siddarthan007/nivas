...
export const saasSettingsController = new Elysia({ prefix: '/saas' })
    ...
    .get('/ai', async ({ user }) => {
        ...
        const usage = await AiService.getUsageToday(user.hotelId);
        return createResponse({ available: true, ...(await SaasSettingsService.getAiConfig(user.hotelId)), usage }, 'AI config');
    }, {
        isSignedIn: true,
        hasPermission: PERMISSIONS.SYSTEM.MANAGE_SETTINGS,
    ...
    .patch('/ai', async ({ body, user }) => {
        ...
        isSignedIn: true,
        hasPermission: PERMISSIONS.SYSTEM.MANAGE_SETTINGS,
        body: t.Partial(t.Object({ enabled: t.Boolean(), model: t.String(), apiKey: t.String(), dailyLimit: t.Number({ minimum: 1, maximum: 10000 }) })),
