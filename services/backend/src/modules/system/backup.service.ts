...
interface PlatformSettingsWithBackup {
...
export const BackupService = {
    async getSettings() {
        const ps = await db.query.platformSettings.findFirst({ where: eq(platformSettings.id, 1), columns: { backupConfig: true } });
        const cfg = (ps?.backupConfig || {}) as BackupConfig;
        return { autoEnabled: !!cfg.autoEnabled, frequency: cfg.frequency || 'WEEKLY', lastRunAt: cfg.lastRunAt || null, keep: KEEP };
    },

    async patchConfig(patch: Partial<BackupConfig>) {
        const ps = await db.query.platformSettings.findFirst({ where: eq(platformSettings.id, 1), columns: { backupConfig: true } });
        const next = { ...((ps?.backupConfig || {}) as BackupConfig), ...patch };
        await db.insert(platformSettings).values({ id: 1, backupConfig: next } as { id: number; backupConfig: BackupConfig })
    ...
    async listObjects(): Promise<{ name: string; size: number; lastModified: Date }[]> {
        ...
        await new Promise<void>((resolve, reject) => {
            ...
            stream.on('data', (o: MinioObject) => { if (o.name) out.push({ name: o.name, size: o.size, lastModified: o.lastModified }); });
    ...
    async deleteBackup(filename: string) {
        ...
        await ensureBucket();
        await client.removeObject(BACKUP_BUCKET, filename);
        return { deleted: filename };
    },

    async refreshDownloadUrl(filename: string) {
        if (!filename || filename.includes('..') || filename.includes('/')) {
            throw new Error('Invalid backup filename');
        }
        return { filename, downloadUrl: await presignedUrl(filename) };
    },
    ...
    async emailAdmins(filename: string, sizeBytes: number, downloadUrl: string) {
        const ps = await db.query.platformSettings.findFirst({ where: eq(platformSettings.id, 1) }) as PlatformSettingsWithBackup | null;
