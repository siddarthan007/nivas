    ...
    .get('/status', async ({ user }) => {
        ...
        const todayAudit = await db.query.nightAudits.findFirst({
            where: and(
                ...
                eq(nightAudits.auditDate, auditDateStr)
            )
        });

        const ranToday = todayAudit?.runAt
            ? new Date(todayAudit.runAt).toISOString().split('T')[0] === today
            : !!todayAudit;

        return createResponse({
            completedToday: ranToday,
            auditBusinessDate: auditDateStr,
