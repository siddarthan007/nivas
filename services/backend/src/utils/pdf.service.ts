...
import { PlatformSettingsService } from '../modules/saas/platform-settings.service';
...
export const PdfService = {
    async fetchImageAsDataUrl(url: string): Promise<string | null> {
        ...
        const stored = await StorageService.readBufferByUrl(url);
        if (stored) {
    ...
    generateInvoiceDefinition(data: any): TDocumentDefinitions {
        ...
        if (logoDataUrl) {
            brandStack.push({ image: logoDataUrl, width: 72, margin: [0, 0, 0, 6] });
        }
        brandStack.push(
            ...
            content: [
                ...
                // Billed To
                {
                    stack: [
                        ...
                        ...(inv.guestEmail ? [{ text: `Email: ${inv.guestEmail}`, style: 'entityDetail' }] : []),
                        ...(data.room?.number ? [{ text: `Room: ${data.room.number}${data.room.type ? ` (${data.room.type})` : ''}`, style: 'entityDetail' }] : []),
                        ...(inv.checkIn && inv.checkOut ? [{ text: `Stay: ${inv.checkIn.slice(0, 10)} → ${inv.checkOut.slice(0, 10)}`, style: 'entityDetail' }] : []),
                    ],
                    margin: [0, 0, 0, 18],
                },
                ...
                ...(h.invoiceTerms ? [
                    { text: 'Terms & Conditions', style: 'sectionLabel', margin: [0, 0, 0, 4] },
                    { text: h.invoiceTerms, style: 'footer', margin: [0, 0, 0, 14] },
                ] : []),
    ...
    async generateSaasInvoiceDefinition(data: any): Promise<TDocumentDefinitions> {
        // Fetch billing config from platform settings
        const billingConfig = await PlatformSettingsService.getBillingConfig();
        ...
        // Use billing config or fallback to defaults
        const brandName = billingConfig.name || 'NIVAS PMS';
        const brandAddress = billingConfig.address || 'Kathmandu, Nepal';
        const brandEmail = billingConfig.email || 'support@nivaspms.com';
        const brandPhone = billingConfig.phone || data.contactPhone || '+977-1-4XXXXXX';
        const brandPan = billingConfig.pan || '123456789';
        const brandVat = billingConfig.vat || '987654321';
        const billingEmail = billingConfig.email || 'billing@nivaspms.com';
            ...
            content: [
                    ...
                    columns: [
                        {
                            ...
                            text: [
                                ...
                                { text: `\n${brandAddress}`, style: 'brandAddress' },
                                { text: `\n${brandEmail} | ${brandPhone}`, style: 'brandAddress' },
                                { text: `\nPAN: ${brandPan} | VAT: ${brandVat}`, style: 'brandAddress' }
                            ]
                        },
                        {
                            width: 'auto',
                ...
                },
                {
                    stack: [
                        ...
                        { text: `4. For billing queries contact: ${billingEmail}`, style: 'termItem' }
                    ],
                    margin: [0, 0, 0, 24]
                },
